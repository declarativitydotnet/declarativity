...
\paa{rename me}
Clients generate {\em cart\_action} and {\em checkout} messages, and send them
to one of the server replicas. Each request is tagged with a request identifier
drawn from a sequence {\em s}, which allows requests to be idempotent.
...
of {\em Cnt} in {\em log} at the first insertion. By making the {\em response} message conditional on {\em log\_event}, we can be sure that only a single response will be sent.
...
If, on the other hand, ignoring late {\em cart\_action} tuples is not acceptable,
...
abstracted away.  Hence asynchronous replication of {\em cart\_action} is confluent,
