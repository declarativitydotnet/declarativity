...
messages are delivered, all replicas will have the same state. \jmh{care to rule out coordination in the definition?  Should you cite Vogels on this and talk more about what flavor of E.C. you mean?}
...
or incorrect  \jmh{Did we establish that commutative $\Longrightarrow$ monotonic?  I think you just asserted that}.  Second, such a program must be
...
and reason about the degree to which it is ``eventually consistent.''  Our first \jmh{implementation of the shopping cart in \lang uses}
...
(checkout), the monotonic store is summarized and sealed \jmh{I think the terms ``store'' and ``seal'' will not translate for some readers.}.  The second shopping cart
...
idempotence) rather than attempting to provide a read/write storage substrate that can provide
such guarantees in general~\cite{quicksand, beyond}.  \jmh{Next setence is broken.}  For example, Dynamo~\cite{dynamo} 
is a read/write storage system
...
cart merging logic.
\jmh{I'm still unhappy with the above paragraph as Peter describes below.}

\paa{joe writes:  This para is a bit muddy.  Point is something like: eventually consistency comes from the app, not the use of storage.  So although  E.C. is often discussed in the context of storage managers (e.g. noSQL), it's more properly a whole-program property that requires appropriate language support to check/enforce.  A language like Dedalus can help make the checks/enforcement easier.
}
...
\paa{a bit of this is redundant with the new section 2}
...
{\em action\_cnt} falls at a non-local stratification boundary,
\paa{ring the bell of whatever name we give such boundaries in section 6}
...
corresponds exactly to a network boundary).  any conclusions drawn from cart\_update
