...
(checkout), the stored state is summarized and ``sealed.''
\jmh{I think the terms ``store'' and ``seal'' will not translate for some readers.}.  
The second shopping cart
is implemented in an imperative, overwriting style: as updates are received, the current copy 
is replaced with the new copy.  We then apply the analysis presented in Section~\ref{properties} 
...
constructs introduced in Section~\ref{sec:constructs} to achieve the necessary coordination,
