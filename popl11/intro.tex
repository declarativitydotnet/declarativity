...
to run in an interleaved fashion.\nrc{I don't follow what ``duplicating the sequential control structures'' means.}\rcs{Two options: something about duplicating the stack, or ``... by explicitly partitioning computation across CPUs, and allowing them to ...''}
...
depth~\cite{Mao2009,navarro}.\rcs{Furthermore, a wide range of network protocols have already been proven correct in higher level logic languages, such as TLA.  Unfortuntately, while these languages are ``executable'' in some sense, they include constructs that cannot be automatically mapped into efficient implementations.}
...
constructs \wrm{assignment of timestamps is operational.  i think the point we want to make is that we elegantly minimize the operational semantics}.  \lang is a subset of \rcs{Equivalent to?} a language with well-studied
features: Datalog enhanced with negation, aggregate functions, choice,
and a successor relation.  \wrm{its not well studied, that's why we're studying it in this paper} \lang provides a model-theoretic \rcs{what does this mean?  Are we just dropping buzzwords here?} foundation
