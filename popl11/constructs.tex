...
to be broken is to add a \dedalus{p\nega} \wrm{\dedalus{p\nega} not defined} subgoal to the body of a simple
persistence rule:
...
\paa{what is a good name for this?}
...
the necessary amount \paa{awk, sorry} to ensure totally ordered processing.  Because
...
We provide the macro {\em ordered\_queue}, which takes arguments identical to those
accepted by the serializer macro; e.g., \dedalus{ordered\_queue[update, 4, 3]}.
