...
duplication of sequential control structures.
%concurrency.  \paa{I think 'concurrency' is the core issue that is addressed
%by our contribution.  'parallelism' is a correllary -- right?}
Unfortunately, this programming style leads to an overspecification of order,
much of which %much of this order is {\em overspecified} --
%\paa{why false?  the point is it's overspecified in the order domain.}
may obscure parallelism, or not be semantically meaningful to the computation.
%, or obscures parallelism. 
We argue for an inversion in design -- parallel by default, and
programmer-defined order only when required for correctness.  
\paa{alternate text: 
Distributed systems are traditionally implemented in imperative languages
using a threaded or event-driven programming style, and inherit these languages'
implicit ordering of both data and control structures.  Concurrency is achieved by
duplication of sequential instructions, and reasoning about the behavior of such systems 
requires explicit consideration of the interleaving of instructions and the arrival order of
messages.  We observe that in general such order is overspecified and not semantically 
meaningful, and argue for an inversion of design for programming distributed systems: unordered and concurrent by default, 
with explicit, programmer-defined order only when order is required for correctness.
}
To this end, we
propose \lang, a {\em logic} language, and study it in the context of
distributed asynchronous systems. \lang is based on the well-studied
...
in the face of {\em asynchrony}, or non-determinism in temporal order.  We
build a library of ordered constructs in \lang that encode a variety of useful
ordered structures.  We also extend static analyses from the Datalog domain,
including a surprisingly powerful analysis that can indicate locations in the
program where a programmer may need to enforce order to maintain determinism in
the face of asynchrony.  We illustrate the benefits of our proposed programming
style through a case study of a shopping cart application with replicated
state.
