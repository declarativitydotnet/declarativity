...
programmers to specify systems as a set of invariants over local state \wrm{local isn't important, it's just a design choice of dedalus}, and
rules that describe how state changes and moves across a network \wrm{again this detail isn't important, it's just a design choice of dedalus}.  This
approach leads to succinct, executable specifications whose faithfulness to the original pseudocode may be visually verified \wrm{well...}.  Moreover, many 
...
reasoning~\cite{boom-techr} \wrm{wat is assertional reasoning?}.
...
Despite their problematic logical interpretation, events and hard state tables
\wrm{maybe too technical -- we should say something like "any sensible system
requires updatable state".  people will believe that.} are desirable primitives
for implementing distributed systems.  Network messages and timers are
naturally represented as ephemeral tuples that are only instantaneously true
\wrm{i see this for timers, but i don't see it for network messages.  why can't
network messages be persisted? also, i think the point we want to make here is
that distribution == data partitioning, and remote derivation induces
communication.}, but which may be joined with persistent state to deduce new
...
\emph{queues}~\cite{netdb} \wrm{``counters'' sound a bit dangerous to me.
first, in general they require addition/entanglement, right?  also, i'm not
sure if it's an idiom.  it seems like it might just be a common implementation
trick (protocol) to express things like distributed universal quantification or
distributed counting quantification. Datalog doesn't need ``counters'', because
you can do universal quantification using stratified negation, and you can do
counting quantification using the ``count'' aggregate.}, and the interaction of
events and hard state allow the simulation of ``soft state'' \wrm{maybe it
would be helpful to say that soft state is a special case of mutable persistence: you mutate after a given time (clock tuple)} useful for best-effort
caching in network protocols and heartbeat messages in distributed
systems~\cite{boom-techr}.  For example,  an Overlog sequence might be
