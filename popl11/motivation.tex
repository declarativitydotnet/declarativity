...
write performance.  To ensure this form of consistency, we need to ensure that our application possesses the 
...
\wrm{maybe ``serial'' would be a better term here} 
...
this implementation is insensitive to message reordering concurrent and simultaneous receipt of messages.  \wrm{we need to mention earlier that we're concerned about this}
...
event of simultaneous receipt) is returned by each call; we also assume messages are handled atomically. \rcs{make a dig about how actual analyses can't assume this stuff in practics and are therefore really, really hard?}
...
RHS~\cite{Loo2009-CACM}.  In distributed environments, network delays
and failures prevent such rules from acting as true logical
implications; existing distributed logic languages model such issues
outside of logic which frustrates static analysis approaches, and
complicates language semantics.  For the purposes of this section,
...
\dedalus{action} with additional columns associating each event with a
server and a request id.  The problem comes in the definition of \dedalus{counter}, which is meant to be a source of unique timestamps; the presence of a request id in \dedalus{counter} corresponds to a fact that is forever true, as does the presence of a client request in \dedalus{action}.  Therefore, naive attempts to bump the counter once per message arrival:
...
lead to infinite models ($\dedalus{s} \equiv \mathbb{N}$ once one action occurs), and leave the underlying problem unsolved.

Existing distributed logic languages resort to operational semantics
at this point; a common approach is to wrap a Datalog interpreater in
...
associated with rule conclusions.  To give a flavor of our subsequent discussion, the second two rules of the above program \rcs{it's broken into two chunks above... what to do???} would be written in \lang as:
