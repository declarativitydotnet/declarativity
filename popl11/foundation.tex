...
Now we can devise \rcs{enumerate?} a unique series of models, which we call the {\em incremental
...
Herbrand Universe is finite.\rcs{what if each model is finite, and has one more symbol than the one before it, and there are an infinite number of incremental models?}
...
make safe use of entanglement \wrm{pointer to lamport, pointer to sequences}.
Additionally, all programs in this paper make safe use of summary aggregates.
