...
operator}.  
\paa{Datalog doesn't have a LFP 'operator' -- it implies LFP with syntactic recursion.
you mean it's equivalent to FO(LFP)?}
...
predicate.'' \paa{why quoted?  also -- "for each assignment of the body, given the 
model -- right?}
...
like \dedalus{count}.
\paa{doesn't this notation seem to mean that only one aggregate per rule is allowed?}
...
\dedalus{time}.  We exclude message loss in this paper to simplify the
discussion, by implicitly assuming that some protocol exists between the client
and server to ensure reliable delivery.  \wrm{timers? cite overlog? boom?}
...
Specifically, we restrict the forom of predicates and rules in the following ways:
...
temporal rules, to indicate the change in time suffix.  A temporal head
...
rules during a particular execution a {\em trace} of the program.  It is easy
to see that each {\em trace} is associated with exactly one model of the
program.
