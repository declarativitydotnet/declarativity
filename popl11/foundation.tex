...
head predicate {\em depends on} each of its body predicates.  \jmh{Need to introduce facts here.}  If a predicate
...
rule's body atoms.\jmh{I don't like this definition or the use of the word ``operator''.  Can't we refer to these as a set of built-in infinite relations, and mention a standard program check to ensure that their infinity does not leading to an infinite model for the program.}
...
universe of constants} of the instance.  \jmh{The preceding sentence was pulled out of a hat.  What is a ``universe of constants of an instance''??}

\subsection{Aggregation}

{\em Aggregates} are functions from sets to constants \jmh{scalar values?  You don't mean constants, at least not in the C sense.}.
...
count of unique {\tt ReqId} values with the last attribute: \jmh{is it clear that Type is one of ``insert'' or ``delete''?}
