...
\paa{the previous sentence is weird since we established these far back in the draft.
perhaps remind the reader "having established the existence and uniqueness of an
ultimate model by ensuring Tstrat and Tsafety" or somesuch}
...
ultimate model is invariant under the operations of combining two adjacent
timestamps, or splitting a timestamp into two separate timestamps.
%
\end{definition}
...
\{1,2\} \forall F \subset \mathcal{H} \exists s \in$ \dedalus{time} $: F@t \subset T_i \land F \subset T_i \cap T_j \implies F@s \subset T_j$.
%
\end{definition}

%Clearly, all traces of an associative \lang instance e the same associativity 
%grouping
%iff the instance is associative.
...
Assume the other direction is false.  This implies that for $R_1, R_2$ traces
of a monotonic \lang instance $I$ with the same associativity grouping,
...
\wrm{fix this} (although possibly at different times),
because if it were not, then $g$ would not be the ``first'' such $f$.  The fact
that both traces have the same associativity grouping is important, because
of \lang's syntactic restriction that rules must unify on time.  Thus, even
under different assignment of timestamps, $g$'s transitive support still derives
$g$ in $R_i$.  Thus, we have reached a contradiction.
\wrm{toughen up, note that we have proven something stronger than the lemma}
%
\end{proof}
...
\dedalus{p}, or is co-incident with \dedalus{p}, then \dedalus{\(\lnot\) q(A)} is false, and the ultimate model
...
commutativity even in the face of non-monotonicity.  
\paa{well, either that or accept the ND and perhaps take steps to ensure singularity 
of the result via, eg, committed choice}
One simple type of
