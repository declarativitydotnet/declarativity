...
\begin{lstlisting}[caption=Quorum vote in Bloom.,label=lst:bloom-quorum-set]
QUORUM_SIZE = 5
...
\end{minipage}\hfill
\begin{minipage}{.48\textwidth}
\begin{lstlisting}[caption=Quorum vote in \blooml{}.,label=lst:bloom-quorum-lattice]
QUORUM_SIZE = 5
RESULT_ADDR = "example.org"
...
(lines~\ref{line:bloom-quorum-msg-start}--\ref{line:bloom-quorum-msg-end}). This
program resembles a ``quorum vote'' subroutine that might be used by an
implementation of Paxos~\cite{part-time} or quorum
replication~\cite{Gifford1979}.
