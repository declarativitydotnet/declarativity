...
\begin{algorithm}
   ...
   \ENDFOR
   \FORALL{rule $R$}
      \FORALL{persistent predicate $P(A_1, \ldots, A_n)$ in $R$'s body}
         \STATE{substitute $P\_store(A_1, \ldots, A_n)$ for $P(A_1, \ldots, A_N)$}
      \ENDFOR
   \ENDFOR
...
Observe that Algorithm~\ref{alg:tsn} binds the time suffix of each rule by replacing it with a cont