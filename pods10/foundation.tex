...
\mathbb{Z}: i < \perp$.  We define the \dedalus{successor} relation, whhere
\dedalus{successor(i,j)} means that \dedalus{j} is the least element that is
greater than \dedalus{i}. \wrm{we have a problem here, we need $\perp$ to be in
the successor relation somehow.  maybe \dedalus{successor($\perp$,$\perp$)}.}
...
form. \wrm{serious problem here -- remove or rewrite}
\paa{what's the issue?  I've defined 'atemporal predicates' in section 2 without having to reference EDB, so I thought we were ok.}
...
%%whether the rule is inductive or
%%asynchronous.  
...
and stands in for \dedalus{successor($\Tau$,S)} in the body.
