...
\dedalus{r\_pos($A_1$,$A_2$,[...],$A_n$) \(\leftarrow\)
r($A_1$,$A_2$,[...],$A_n$);}
...
In a well-formed \slang rule, every subgoal uses an existential variable
$\Tau$ as its time suffix, and $\Tau$ must not be
...
suffix can be thought of as a fact that is true ``at time $C$''.  Deductive
