...
\jmh{Argh, where did this predicate name prefix shit come from?  Also, can we do this without horizontal partitioning, as we described previously?  Trivial solution is to put Agent into the relation name but then you need a head per neighbor.  Ick, and uh-oh.}

\jmh{what's the ``program'' construct?  do you need it?  if so, define it.}
\begin{Dedalus}
...
The table \emph{members} is a persistent relation defined in another module \jmh{what's a module??} and containing the broadcast 
membership list.  
...
\jmh{Can you comment on the careful mix of synchrony and asynchrony here?  Seems like that's the payoff of this example discussion.}
