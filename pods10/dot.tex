...
appear in the head of any rule in the program.  The extensional database or EDB
is the set of facts in extensional relations.
%
\end{definition}
...
a rule, and never in the body \wrm{then, how do we write inductive rules?}.  In
a Dedalus program, we use $r$ as shorthand for $r\_pos$ (as the true EDB
...
Since an EDB fact may not appear in a rule's head, events come from sources
