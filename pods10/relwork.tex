...
derivations.  Similarly, Timed Concurrent Constraint Programming (TCCP)~\cite{tccp, tdccp} handles nonmonotonic constructs in a CLP framework
by outputting a new (possibly diminished) store and constraint program at each timestep.  Lamport's TLA+~\cite{tla} is a language for specifying 
...
Significant recent work~\cite{boom-techr,boon, p2, loo-sigmod06} has focused on applying deductive database languages extended with networking 
...
Unfortunately, the declarative language gives no hints as to how to and when deletions and updates
