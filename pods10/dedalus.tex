...
\begin{definition}
A \emph{minimal trace} is a projection of a trace that excludes any events caused by inductive rules, and transforms the event times
...
A trace has only one minimal trace, but an infinite number of infinite traces have the same minimal trace: the minimal trace thus forms an 
equivalence class among traces.  The minimal trace of the trace instance given above is:
