...
\item $N$
\item $N + 1$
\item $r(A_{1}, A_{2}, [...], A_{n}, N)$
\item an integer
\end{enumerate}

\wrm{wait, it's not really N + r(), right?  it's something like receiver\_N + r(), because logical clocks at different nodes may be arbitrarily different, and run at arbitrarily different speeds, right?  (i.e. each node has a different mapping of ``real time'' to logical time).  

a bad operational way to evaluate this would be to have the receiver send its value of receiver\_N.  however, any request to the receiver to send receiver\_N sent within the dedalus model is also a message subject to network delay.  thus, there's a ``time bootstrapping'' problem here that needs to be solved somehow.  introducing a top-down component to the evaluation, such as enabling nodes to pull from each other could solve this, and some now() function could also solve this.

a less bad operational way to evaluate this would be to have the sender trivially evaluate r() to always be 1 -- allowing the network delay to introduce nondeterminism -- and allowing the receiver to fill in receiver\_N.  this still runs in to a similar problem with assigning time.}
...
Rules with the head suffix $r(\_)$ are also temporal rules, but unlike inductive rules, they carry no guarantee as to in which timestep, if any, 
