...
Traditional Datalog interpreters work against static databases, and would attempt to enumerate these values, which would make this approach impractical.\rcs{still don't like prev sentence..}  
...
of \dedalus{successor} at runtime \wrm{whats pailiative (helping fix the problem) here???}.  This section
...
Datalog\rcs{what should we call this?} program is unsafe.  
%
\end{example}
...
A rule of kind 2 does not entail an atom in timestamp $T$ if that atom does not
exist at $T-1$.  Rules of kind 1 and 3 may entail atoms in timestamp $T$ not in
$T-1$.
...
Rules of kind 1 may produce deductions forever.  Recall that every predicate in
\slang is either instantaneous or inductive.  No instantaneous predicates exist
at time 
...
\rcs{i found the example hard to read, and don't see how it's fundamentally different from this, which I think is easier to understand:}
