...
the same assignment of constant to attributes for all attributes except the
time suffix.
%
\end{definition}
...
a temporally unsafe program changes infiniely.  Note that
the \slang program in Example~\ref{ex:tempsafe} is temporally safe because
\emph{r1} and \emph{r2} both satisfy the second condition above.

\begin{lemma} 
%
A \slang instance with finite EDB where every rule satisfies conditions 1-3
above is indeed temporally safe.
%
\end{lemma}
%
\begin{proof}
...
Since the EDB is finite, a rule that satisfies condition 3 above
may only be triggered finitely often.  Thus, every rule $r$ of this kind has
\wrm{DANGER: PROOF IN PROGRESS}
%
\end{proof}
