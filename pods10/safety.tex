...
In the previous section we demonstrated that \slang can capture intuitive notions of persistence and mutability of state via a stylized use of Datalog.  However, the alert reader will notice that even the simplest of these Datalog programs are unusual, since they produce derivations for an infinite number of values of the time suffix.  In traditional Datalog this is cause for concern, but in the context of distributed systems and networks, the need for infinitely-running ``services'' or ``protocols'' is very common. In this section we take traditional notions of acceptable Datalog programs, and extend them in a way that admits sensible distributed services and protocols.
...
\rcs{check this paragraph}

Since our definition of \dedalus{successor} violates these rules, and
indeed leads to an infinite set of facts, no \slang program meets this
defintion of safety.  However, \dedalus{sucessor} models time, not
computation; later sections explain how \lang implementations avoid
enumerating the contents of \dedalus{successor} at runtime.  This
...
The infinte \dedalus{successor} relation complicates our discussion of safety; consider the following \slang program, which derives a single, persistent fact:
...
tuple in \dedalus{successor}.  Since \dedalus{successor} is infinite, the corresponding datalog\rcs{what should we call this?}
program is unsafe.  
%
\end{example}
...
true atoms with time suffix $T$ is equivalent to the set of all true atoms with time suffix $T-1$.\rcs{what is a true atom?  Why distinguish between true atoms and false atoms?  Surely the neg tuples will match as well.  If not, then the program has somehow not quiesced...}
...
EDB facts are true after $T$, then the instance will be henceforth quiescent.\rcs{what does it mean for an edb fact to be true at time $T$?  It would be better to say that there is no EDB fact with timestamp T, since we're trying to ground the discussion without talking about time.}
%
\end{observation}
%
\begin{proof}
...
no inductive rules.\rcs{does e depend on p, or is it the other way?  Does no inductive rules mean that $p$ is not persistent?}
%
\end{definition}
...
may only be triggered finitely often. \rcs{Ok, so $e \succ p$ must mean that the depends-on is opposite the direction of the arrow, since $e$ is in a deductive rule.}
%
\end{enumerate}
...
Intuitively, a temporally safe program terminates \rcs{ used to say quiesces, but temporally unsafe programs can quiesce a finite number of times} given a finite EDB, while a
...
\begin{example}
A \slang instance with a temporally unsafe deductive rule.\rcs{is this even a \slang instance?  Is it a datalog instance?  Perhaps this example belongs earlier in the text, where we define range-restricted.}
...
\rcs{i found the exampe hard to read, and don't see how it's fundamentally different from this, which I think is easier to understand:}
