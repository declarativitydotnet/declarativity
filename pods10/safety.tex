...
depends on $p$), either $p$ appears in the head of no inductive rules, or if the body
of each inductive rule in which $p$ appears contains at least positive instantaneous 
predicate.
...
Note that we do not forbid temporally unsafe programs. \wrm{joe sez "discuss
safety is wil, but-out regured"}
