...
Datalog program in which all predicates have no time suffix.  If all the rules in such
a program are function-free and range-restricted, then clearly all evaluations will
be safe.
...
Inductive rules are not safe.  Consider the \dedalus{successor} relation described above.  According to our
...
\wrm{BROKEN: Need to fix this Herbrand business -- it's not countably infinite.}
...
inferences are henceforth \wrm{bill to insert proof/argument that you get
``henceforth'' from a single state where the program is quiescent} stable.
...
\wrm{buggy text here, come up with motivation that doesn't involve implementation}
%A \slang instance may receive arbitrarily many external input tuples over the
%course of its execution, but should not wait
%arbitrarily long before performing deductions.  In this section we introduce
%the notion of EDB preambles, and prove an equivalence between two types of
%evaluation.
...
preambles with lower indices, we have the monotonicity property:
