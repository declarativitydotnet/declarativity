...
leads to an infinite set of facts, \slang programs \wrm{urulaje???} this
defintion of safety.  However, \dedalus{sucessor} models time, not computation;
later sections explain how \lang implementations avoid enumerating the contents
of \dedalus{successor} at runtime \wrm{whats pailiative here???}.  This section
...
The single ground fact will cause one deduction for each
...
A rule that satisfies condition 2 may not generate a fact in timestamp $T$
if that fact does not exist at $T-1$.  A rule that satisfies condition 3
