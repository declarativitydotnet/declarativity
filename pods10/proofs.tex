...
Having adapted the notion of safety to a temporal context, it is only natural to turn our attention to another traditional Datalog issue: semantics of programs with negation.  As we will see, the inclusion of time introduces a ``source of monotonicity'' in programs that allows for clean minimal model semantics in some surprising cases, and also provides us with an ability to do purely syntactic checks for monotonicity of a broad class of temporal programs.
...
\rcs{joe said this once. the \slang program has a unique, infinite minimal model.  As the flip flop example shows, this doesn't mean that it quiesces.  We should make the sleight of hand explicit.}
...
unique minimal model, as any Datalog instance %without choice
has a unique
minimal model. 
...
stratifiable.  Fortunately, we are able to define a syntactically checkable notion of
{\em temporal stratifiability} of  \slang programs that maps to a subset of {\em modularly
...
additional cycles\rcs{except in the definition of successor} or negation.  Since the time suffix in the head of an
...
Therefore, we have $p_{n} \not\succ^* del\_p_{n} \not\succ^* p_{n+1}$; informally, earlier values do not depend on later values.
