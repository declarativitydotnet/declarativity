...
that our execution environment ensures/assumes \rcs{XXX} some sort of
monotonicity property, and that with monotonicity of time, we get
...
execution of a \lang program (which may be non-determinstic due to
\@async) is equivalent to the execution of some deterministic \slang
program; causality constraints reintroduce the temporal straifiability
...
distributed systems built atop ureliable and asynchronous primitives
could very well observe seemingly nonsensical orderings; one
possibiity is that, due to caching or failure, comments begin to
appear to arrive well before the blog post they are attached to.
