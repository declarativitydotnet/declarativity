...
A principal motivation for \lang is modeling and implementing distributed systems.
To represent such a system we must consider a multiplicity of agents evaluating local programs and
asynchronously communicating via messages.  As with previous distributed logic languages, we model messages as
...
at each agent according to our definition, there is no reason to assume that a subset of {\em successor} 
relevant to computation at any particular agent will intersect a relevant subset at another.  In other words,
if we consider the ``current local time" as a pointer to a particular row of successor, each agent's pointer will likely indicate a different row in a distributed system.
\lang does not allow an agent to know the current time of another.

It follows that a message rule as described above will deduce a head predicate whose timestamp is independent
of the timestamps in the body predicates.
...
p(A, B)@\rcs{persistent} \(\leftarrow\) p\_wait(A, B, N)@M, N < M;
\end{Dedalus}
