...
in \emph{$X_2$} for each distinct set of variables in \emph{$X_1$}.\rcs{there are multiple sets of variables in x1, so x1 is a set of sets?}
...
from a value in the special \dedalus{successors} predicate, which is defined using the following datalog rules:
...
Intuitively, \dedalus{successors(S, T)} is true if T happens after S
...
Each asynchronous rule has the following additional varibales in its
body: \dedalus{successors($\Tau$, S), choose(($\Tau$), (S))}, where, as above,
...
\begin{Dedalus}
r(A, B, S) \(\leftarrow\) e(A, B, \(\Tau\)),
  successors(\(\Tau\), S), choose((\(\Tau\)), (S));
\end{Dedalus}
\end{example}
...
\rcs{do we need to prefix it with ``asynchronous''?}
