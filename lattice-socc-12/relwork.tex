...
two kinds of CRDTs: ``operation-based'' types (called CmRDTs) and
``state-based'' types (called CvRDTs). \lang is most closely related to CvRDTs
(the state of a CvRDT replica must be a semilattice), although the two CRDT
variants are equivalently expressive. Shapiro et al.\ provide a formal model for
replica convergence and detail a catalog of practical CRDT designs. CRDTs and
