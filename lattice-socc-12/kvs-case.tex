...
\paa{my confusion wrt COPS is even more acute here: if there exists a merge
function for *any* pair of values (which must be true if the values are a
lattice), why on earth did we care about causal order to begin with?}
...
Note that although the \emph{fst} of an \texttt{lpair} will increase over time
...
\paa{if R+W > N we don't need vector clocks or values drawn from a lattice,
right?  so the code described in sections 5.1 and 5.2 is only active in the 
"EC mode" -- yes?
}
...
\paa{need to check the ref, but I thought read repair implies read operations 
actually mutating (repairing) replica state, not just client-side 
reconciliation}
