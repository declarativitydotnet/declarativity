...
to logical clocks. Let $V_e$ denote the vector clock associated with event $e$;
let $V_e(n)$ denote the logical clock of node $n$ in $V_e$. If $V_e < V_{e'}$,
$e$ causally precedes $e'$, where $V_e < V_{e'} \equiv \forall x [ V_e(x) \leq
V_{e'}(x) ] \land \exists y [ V_e(x) < V_{e'}(y) ]$.

In \lang, a vector clock can be represented as an \texttt{lmap} that maps node
identifiers to \texttt{lmax} values. \texttt{lmax} is appropriate because the
