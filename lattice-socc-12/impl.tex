...
function against the complete database state in each round.  \jmh{I expected the example to focus on a duplicated element---where is that?}.
...
various sizes---in a graph with $n$ nodes, each node had $O(\log_2 n)$ \jmh{Why Big-O?} outgoing
