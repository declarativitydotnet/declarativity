...
All applications that manage replicated state must ensure that consistency
is maintained.
Database programming models ensure strong consistency guarantees by exposing
...
the recent interest in ``eventually consistency,'' a programming model that
...
In this paper, we introduce \emph{Bloom}, a new programming framework intended
to simplify the design of distributed systems with ``loose'' consistency
requirements.  {\em Bloom} includes a new language and runtime (called {\em
  Bud}), and a static analysis technique that identifies {\em points of order},
which correspond to parts of the program that may require programmer attention
to ensure consistency in the face of message reordering.  {\em Bloom}'s design
%is motivated by Hellerstein's CALM Conjecture, which provides a partially
%descriptive and partially semantic characterization of eventually consistent
%programs.
is inspired by database query languages and analysis techniques from the logic
programming literature.  To demonstrate these techniques, we implement variants
of a distributed ``shopping cart'' application and analyze them to discover
points of order.  We evolve the applications in reaction to these hints, and 
...
%insensitive to reordering and message delay.  Using the CALM conjecture \wrm{better be a theorem, because we need to prove it in this paper},
