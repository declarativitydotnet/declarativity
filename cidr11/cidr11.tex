...
In many distributed programming circles today, conventional wisdom rejects transactional solutions in favor of application-level reasoning about eventual consistency.  But there are few concrete principles or tools to help programmers think through and assess the trustworthiness of such reasoning.
...
We address this void with the CALM principle, which connects distributed consistency to logical tests for monotonicity.  We then introduce \emph{Bloom}, a distributed programming language that is amenable to high-level consistency analysis, and encourages order-insensitive programming.  We present a prototype implementation of Bloom as a domain-specific language in Ruby. We also present a static analysis technique that identifies {\em points of order} in Bloom programs: code locations where programmers need to inject coordination logic to ensure consistency.  
%{\em Bloom}'s design
...
We present variants
of a distributed ``shopping cart'' application in Bloom, and analyze them to discover
points of order.  We show how Bloom's analysis and code structure allow us to evolve the applications. Performance implications are illustrated by running the code on the EC2 cloud platform.
