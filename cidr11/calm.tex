...
\jmh{distributed non-monotonicity requires waiting for the full set.  this is hard.}
...
\jmh{the solution to ``waiting'' is coordination, and its not needed otherwise.}
...
% Our language, {\em Bud}, omits coordination logic
...
\jmh{if we start by default without coordination, we can do simple syntax checks for non-monotonicity}
...
\jmh{these syntax checks can be isolated within the program, and resolved via coordination logic}
