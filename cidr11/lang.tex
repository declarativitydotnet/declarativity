...
\texttt{budcllection}, which inherits Ruby's built-in \texttt{Enumerable}
...
\begin{comment}
\jmh{We should probably add a table of budcllection methods, and a mention of
  ...
  budcllections, i.e. join and its variants (natjoin, leftjoin).}
\end{comment}
...
\begin{figure}
	...
	\begin{tabular}{|p{0.65in}|p{2.35in}|}
		\hline
		BudCollection Method & Description\\
		\hline
		\texttt{bc.map} & Inherited from Enumerable, takes a code block as an argument, and returns the collection formed by applying the code block to each element of \texttt{bc}\\
		\texttt{bc.flat\_map} & Inherited from Enumerable, takes a code block and returns the collection formed by the concatenated (flattened) results of running block once for every element in \texttt{bc}.\\
		\texttt{bc.reduce} & Inherited from Enumerable, takes an initial value, \texttt{memo} variable and code block, and applies the block to \texttt{memo} and each element of \texttt{bc} in turn.  Synonym for \texttt{bc.inject}\\
		\texttt{bc.empty?} & Inherited from Enumerable, returns true if \texttt{bc} is empty.  Equivalent to negation in Datalog.\\
    \texttt{bc.include?} & Inherited from Enumerable, takes an object as an argument and returns true if that object is equal to an element of \texttt{bc}.\\
		\texttt{bc.group} & Takes a list of grouping columns, a list of aggregate expressions and a block, computes the aggregates for each group, and applies the block to each group/aggregation result.\\
		...
		\hline\hline
		\texttt{join, leftjoin, outerjoin, natjoin} & Methods of the Bud class to compute join variants over BudCollections.  \texttt{join}, \texttt{leftjoin} and \texttt{outerjoin} take an array of BudCollection names to join, as well as a variable-length list of arrays of join conditions.  The natural join \texttt{natjoin} takes only the array of BudCollection objects as an argument.\\
		\hline
	\end{tabular}
\end{small}
