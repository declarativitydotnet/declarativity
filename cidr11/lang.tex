...
assumptions about order.  \kc{Therefore, it is no...}It is no surprise therefore that popular imperative
...
\kc{got confused by references to figure 5.}
...
%%\texttt{id} across all tuples in collection \texttt{q}.  
of unique \texttt{reqid} values for every combination of values for \texttt{session}, 
\texttt{item} and \texttt{action}.
%User-defined aggregates can be specified via a \texttt{reduce} method from Ruby \texttt{Enumerable}.  
...
%Line 16 of Figure~\ref{fig:queue} gives an example of the natural join (equijoin on matching %column names, in this case \texttt{id}.)
...
\kc{Would it make sense to move this paragraph just before Bloom basics?}
