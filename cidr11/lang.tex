...
\begin{figure}
	...
	\begin{tabular}{|p{0.65in}|p{2.35in}|}
		...
		\texttt{bc.reduce} & Inherited from Enumerable, takes an initial value, \texttt{memo} variable and code block, and applies the block to \texttt{memo} and each element of \texttt{bc} in turn.\\
		\texttt{bc.empty?} & Inherited from Enumerable, returns true if \texttt{bc} is empty.  Equivalent to negation in Datalog.\\
    \texttt{bc.include?} & Inherited from Enumerable, takes an object as an argument and returns true if that object is equal to any element of \texttt{bc}.\\
