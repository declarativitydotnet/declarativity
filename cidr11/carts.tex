...
module DisorderlyCart
  ...
  def do_checkout
    ...
    status <= join([action_cnt, checkout_msg]).map do |a, c|
		(*\label{line:dis_checkout_negation}*)
    ...
    response_msg <~ status.group(
      [status.client, status.server, status.session],  (*\label{line:dis_checkout_response_beg}*)  
...
function \texttt{accum}, to match the format expected by CartClient (and used in the previous
example in Figure~\ref{fig:dest-cart}).
