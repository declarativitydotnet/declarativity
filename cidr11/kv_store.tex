...
is received, its key-value pair is stored in \texttt{kvstate} at the immediately
following timestep (line~\ref{line:kvs-put}).  If the given key already exists
...
Note that ReplicatedKVS wants to ``intercept'' \texttt{kvput} events from
