...
def summarize
	...
	status <= join([action_cnt, action_cnt, checkout_msg]).map do |a1, a2, c| (*\label{line:dis_checkout_item_beg}*)
		if a1.session == a2.session and a1.item == a2.item and a1.session == c.session and a1.action == "Add" and a2.action == "Del"
			[a1.session, a1.item, a1.cnt - a2.cnt] if (a1.cnt - a2.cnt) > 0 (*\label{line:dis_status_diff}*)
		end
...
Line~\ref{line:dis_action_msg} inserts all client updates into the persistent
table \texttt{cart\_action}.  Line~\ref{line:dis_count_reqid} defines
...
\texttt{status} contains its final quantity: the difference between its number
