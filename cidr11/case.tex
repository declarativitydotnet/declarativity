...
module DisorderlyCart
  ...
  def saved
    ...
    action_cnt <= cart_action.map do |a|
      unless cart_action.map do |c| 
        [c.session, c.item] if c.action == "D"
      end.include? [a.session, a.item]}
...
the contents of the store to multiple servers (e.g., the ReplicatedKVS module shown in 
figure~\ref{fig:kvs-repl}, mixed-in with some implementation of MulticastProtocol).
...
%%of its inputs.
Figure~\ref{fig:pdg-destructive} presents the analysis of the 
