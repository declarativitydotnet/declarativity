...
\paa{TODO: reswizzle the line number pointers when the code quiesces.}
...
dataflow~\footnote{Datalog afficioanados may be troubled by our mention of cycles
...
cart code of Figure~\ref{fig:pdg-destructive}, the underlying key-value store uses the non-monotonic \texttt{$<$-} operator to model updateable state.\footnote{Our full-length paper discusses the non-monotonic nature of \texttt{$<$-}, and expands the SCC to analyze the
