...
The analysis of our disorderly implementation,
...
contents of the response message.  \end{comment} indicates that no points of
order are crossed when a client sends an \texttt{action\_msg} to a replica, and
when replicas exchange \texttt{action\_msg} messages during replication.  Thus,
all replicas will eventually have the same contents in their
\texttt{cart\_action} collections, even in the absence of coordination.  There
is, however, a point of order at checkout, when a \texttt{checkout\_msg}
message is joined with an aggregate over the set of updates.  While the
accumulation of \texttt{cart\_action} is monotonic, an aggregate cannot be
returned until \texttt{cart\_action} is complete.
