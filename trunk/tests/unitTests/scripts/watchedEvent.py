...
def usage():
        print """
                watchedEvent.py -E <planner path> -B <olg path> -T <time in seconds>
...
def script_output(stdout):
	...
	p = re.compile(r"""
		(^[#][#]Print\[RecvEvent: \s* RULE \s* a\]: \s* \[am_alive\(localhost:10000\)\])
		""", re.VERBOSE)
	...
	if flag:
		print "Test passed" 
                ...
                print "Match failed. Line is ", output
		print "Test failed"
...
for key,val in opt:
        if key=='-B':
                ...
                sys.exit(0)
try:
        args=[executable_path , '-o', os.path.join(olg_path, 'watchedEvent.olg'), '2>&1']
