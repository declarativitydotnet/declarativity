...
public class Vis {
    ...
    protected static Hashtable<String, P2Node> nodes = new Hashtable<String, P2Node>();
    ...
    public class P2Node {
        ...
        public ArcConnector make_arc (
                                      P2Node other, boolean exterior, Color color) {
            ConnectorTarget target = new PerimeterTarget();
            Site a = null, b = null;
            boolean succ = in_range_mod (guid,  guid.add (MODULUS.divide (BigInteger.valueOf(2))), 
                                         other.guid, MODULUS);
            if (succ) {
        ...
        public void redraw () {
            ...
            dot = new BasicEllipse (x_pos (), y_pos (), diameter (), 
                                    diameter (), (host==null) ? Color.black : Color.blue);
        ...
        public void redraw_leaf_set () {
            ...
            if (! show_leaf_set || host == null)
                return;
            for (Iterator iter = host.successors().iterator(); iter.hasNext(); ) {
            	HOST h = (HOST) iter.next();
            	String g = h.getGUID();
            	if (g == null || !nodes.containsKey(g)) continue;
            	FigureLayer layer = graphicsPane.getForegroundLayer();
            	ArcConnector conn = make_arc ((P2Node)nodes.get(g), true, Color.green);
            	layer.add (conn);
        ...
        public void redraw_hostname () {
            ...
            if (pie >= hostname_anchors.length)
                pie = 0;
            hostname_label = new LabelFigure (host.getNAME() + " GUID(" + guid_str + ")");
            hostname_label.setAnchor (hostname_anchors [pie]);
            Point2D pt = CanvasUtilities.getLocation (dot.getBounds (),
                                                      CanvasUtilities.reverseDirection (hostname_anchors [pie]));
