	  ...
	  public static class InverseLineReader {
	    ...
	    boolean fillBuffer() throws IOException {
	    	if (this.start < this.position) {
	    		System.err.println("START " + start + " end " + end +  " -- file pos " + position);
	    		this.position = (this.position - start) > bufferSize ? this.position - bufferSize : this.start; 
	    		in.seek(position);
	    ...
	    public int readLine(Text str, int maxLineLength, int maxBytesToConsume) throws IOException {
	    	...
	    	do {
	    		while (bufferPos-- > 0) {
	    			if (buffer[bufferPos] == '\n' || buffer[bufferPos] == '\r') {
	    				if (bytesConsumed > 0) {
	    					bufferPos--; // Skip the new line the next time around.
	    					return bytesConsumed;
	    				}
	    			}
	    			else {
	    				str.append(buffer, bufferPos, 1);
	    				if (++bytesConsumed >= maxBytesToConsume) return bytesConsumed;
	    			}
	    		}
	    	} while (fillBuffer());
	    	return bytesConsumed;
