...
We now describe the declarative rules for performing our version of the Cascades top-down optimizer. 
...
generate subgroups [AB], [BC], [AC]).  Rule~\ol{cs2} in
...
{\bf branch} table triggers rule~\ol{cst1}, which inserts the ``current''
...
subgroup have been explored, and a winner selected, rule~\ol{cst3} advances the
...
Figure~\ref{ch:evita:fig:cascades_plan} gives three rules for generating plans in the
...
of actual plans, as indicated by the $OuterGroup$ and $InnerGroup$ variables in rules~\ol{p2}
and \ol{p3}. 
...
include appropriate rules in the form of Rule~\ol{p1}.
...
such plans to be considered last. Since rule~\ol{p3} considers an index-loop join
