...
Performs the System R dynamic programming optimization on an \OVERLOG program. \\ \hline
...
\ol{path}, etc.  The second rule triggers on \ol{path} event tuples, and joins
with the \ol{link} relation, etc.  Both of these rules project onto the
\ol{path} relation, which in turn triggers further invocations of rule~\ol{R2b}
on new \ol{path} data.
...
\datalogspace {\bf remainder}(@A, Pid, Rid, NewRid, Pos, NewPos, DeltaPos), \\
\datalogspace {\bf sys::predicate}(@A, PredID, Rid, Notin, Name, Tid, \_, Schema, Pos), \\
...
R2b {\bf path}(@X, Z, P, C) :- \\
\datalogspace {\bf link\_copy}(@Y, X, Y, C1), {\bf path}(@Y, Z, P2, C2),\\
\datalogspace f\_contains(X, P2) == false, \\
\datalogspace P := f\_cons(X, P2), C := C1 + C2. \\
...
throughout the entire ~$50414$ line code-base ($10794$ lines of \text{.C} files under
\ol{/p2core} alone).
