...
\text{System R} $(Chapter~\ref{ch:opt:sec:systemr})$ & \OVERLOG  & 
A {\em top-down} dynamic programming optimization. \\ \hline
Cascades $(Chapter~\ref{ch:opt:sec:cascades})$ & \OVERLOG  & 
...
the program text ($text$).  The \ol{program} relation also contains an
...
contain an event predicate e.g., the \ol{delta::programEvent} tuple.
...
an event, and we need to rewrite it.  Once a \ol{rewrite} event tuple has been
...
($\Delta r$).  Rule~\ol{d3} specifically creates the delta predicate, placing
...
copying subgoals rule~$r$ to the new $\Delta r$ rule.  Notice that rule~\ol{d7}
...
On node $Y$, the \ol{link\_copy} tuples trigger rule~\ol{r2b}, which completes
execution of rule~\ol{r2}, sending the \ol{path} results back to node $X$.
