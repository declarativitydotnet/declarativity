...
Starburst~\cite{phh92}, Volcano~\cite{volcano}, OPT++~\cite{opt++}).  We prove
this claim by expressing a variety of traditional and novel query optimizations
in a recursive, declarative language.  These optimization are presented in
subsequent chapters, starting with the magic-sets rewrite in
Chapter~\ref{ch:magic}, and continuing with the System R and Cascades
cost-based optimizations in Chapter~\ref{ch:opt}.  Based on our experience, we
...
up into a {\em compilation stage}.  In Section~\ref{ch:evita:sec:delta}, we
present our first declarative compilation stage (packaged with Evita Raced):
the delta rewrite~\cite{p2sosp}.  Some final thoughts before moving to the next
chapter are given in Section~\ref{ch:evita:sec:summary}.
