...
$k-1$ terms, as shown in the first loop of \ol{bestplan} in
...
identifier, which the \ol{bestPlan} table uses to reference optimal plans.  The
...
%Extending this rule to consider access methods would
%entail adding a predicate that references all access methods to a given
%relation.

Figure~\ref{ch:opt:fig:planseed} shows the optimizer rule that creates the
initial plan for each rule from the event predicate.  A \ol{plan} tuple
represents a query plan for a given rule, and the plan's \ol{size} reflects the
...
\ol{systemr::programEvent} event stream in rule \ol{pg1}, which initiates the
optimization process.  The \ol{systemr::programEvent} tuple is subsequently
...
program, rule \ol{pg1} adds a ``size~$1$'' plan to the \ol{plan} table.  The
...
derived by rule~\ol{pg2} in Figure~\ref{ch:opt:fig:plangen1}.  The rule is
...
($PlanID$) used to select the reference plan in the \ol{plan} relation.  Each
plan tuple contains a list of the predicate identifiers that already appear in
it.  This list is referenced by the $OuterPlan$ variable in the \ol{plan}
predicate of rule~\ol{pg2}.
...
with the \ol{sys::table} predicate.  The selection predicate $PredPos > 0$
...
These new \ol{plan} tuple deductions contain the desired join method
...
subsequently joined with the (additional) \ol{sys::index} predicate, which adds
...
selectivity information given by the $Selectivity$ \ol{sys::index} predicate
...
\datalogspace {\bf sys::rule}(@A, Rid, Pid, \_, \_, \_, \_, TermCount), \\
\datalogspace $PredPos > 0$,\\
\datalogspace {\bf sys::table}(@A, Tid, \_, \_, \_, \_, TCard, TableSort),\\
\datalogspace {\em f\_contains}(PredID, OuterPlan) == $false$,\\
\datalogspace Card   := OuterCard * (TCard / 10),\\
...
$TableSort$ attribute in the \ol{sys::table} plan table identifies the sorting
...
f\_sortJoinAttributes} function.  The \ol{plan} predicate generated by the
...
contained in the \ol{branch} table, and initialized to $inifinty$.  The upper
...
predicates in the rule body.  Rule~\ol{bb2} is triggered when such an update occurs
in the \ol{group} relation.  It creates a \ol{branch} tuple containing the
group information, an initial branch position, and the initial bound for this
branch group.  Rules~\ol{bb3} and \ol{bb4} react to this \ol{branch} update by
...
In this section, we will present our rules for generating \ol{plan} tuples for
the group indicated by a \ol{branch} tuple.  A \ol{plan} tuple is equivalent to
...
the derive \ol{plan} tuples.  Rule~\ol{bp7} handles the case of a single
...
which determines the cost of the optimal plan regardless of order. Rule~\ol{bb12}
follows by determining the best cost plan for each interesting order. The familiar
rule~\ol{bb13} from System R determines the orders that are interesting, based
on grouping and joining attributes. The final two rules identify
the \ol{winner} plans for the given group. In rule~\ol{bb14}, we ensure that
the best overall plan is part of the \ol{winner} relation, and rule~\ol{bb15}
