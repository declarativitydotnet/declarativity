...
set of rule tuples is joined with the \ol{sys::predicate} table along the
...
a \ol{plan} of ``size one'' for each rule in the input program.  The
...
variable in the \ol{sys::table} table identifies the ordered attributes of the
...
plan among the {\em set of} equivalent plans.  The purpose of rule~\ol{sr6} is
to ensure we consider the costs associated with interesting ordered plans.

Rule~\ol{sr8} determines if a plan, ordered by some given attributes, is
...
System R optimizer stage.  Rule~\ol{sr11} counts the number of rules in the
target program.  This count will be used to check for our end condition, which
occurs when all rules have been given a \ol{bestPlan} tuple with a plan size
equal to the number of subgoals.  Rule~\ol{sr12} identifies the completion of a
rule based on this end condition, while rule~\ol{sr13} counts the number of
completed rules for a given program.  Finally, when the counts in
\ol{completeRuleCount} and \ol{rules} are equal (a familiar pattern),
rule~\ol{sr14} generates the termination signal for a given program by
inserting a new tuple into the \ol{program} program with the ``systemr'' stage
name.
...
program.  Rule~\ol{bb2} converts \ol{groupSeed} tuples to \ol{group} tuples by
...
Rule~\ol{bb3} triggers on an update to the \ol{group} relation, creating a
\ol{branch} tuple with the given group identifier, an initial branch position,
and an initial group bound ($\infty$).  Rules~\ol{bb4} and \ol{bb5} create new
subgroups; first (\ol{bb4}) by excluding the predicate at the given branch
position $Pos$, and second (\ol{bb5}) by including just that branch position's
predicate.  As an aside, these two rules would need to be modified in order to
consider ``bushy'' plans.  As we will see in
Chapter~\ref{ch:opt:sec:cascades_plan}, \ol{branch} tuples are used for
generating \ol{plan} tuples.  Here, we must ensure that the plan enumeration
does not update the branch position until all plans relevant to that position
have been discovered.  We detect this condition in rule~\ol{bb6} with the
\ol{branchComplete} predicate: described in
...
Figure~\ref{ch:opt:fig:cascades_plan1} presents two rules for generating
\ol{plan} tuples relevant to a particular branch position.  Rule~\ol{bp7}
...
Rule~\ol{bp8} generates a nested-loop join \ol{plan} using a ``winner'' plan as
...
the \ol{winner} predicate with the \ol{plan} predicate to obtain an actual
(best) plan.  For the inner predicate, we look for a \ol{branch} containing a
single predicate then, using the \ol{sys::predicate} and \ol{sys::table}
predicates, we obtain the desired inner information (i.e., $PredID$, $ISchema$,
and $TCard$ variables).  The parent \ol{branch} is identified by equating the
$GroupID$ to the combined child identifiers: given by $OGroupID$ and
$IGroupID$.  The parent \ol{branch} provides the $Bound$ variable, which is
used here to prune expensive plans.  The final step in this rule creates the
remaining variables needed to project onto the \ol{plan} predicate.  Like our
...
from rule~\ol{bb8} and the respective System R rules~\ol{sr3} and~\ol{sr4}, so
...
handles selection predicates, which resembles rule~\ol{sr5} in
...
from the plans generated for a given group.  We begin with rule~\ol{bb9}, which
determines the cost of an optimal plan, for each group, regardless of its
order.  Rule~\ol{bb10} does the same but also considers interesting orders.
Rule~\ol{bb11} is nearly identical to rule~\ol{sr8}; both determine the orders
that are interesting based on later grouping and joining attributes.  Finally,
rules~\ol{bb12} and~\ol{bb13} select winners based on the costs referenced by
the \ol{bestGroupCost} and \ol{bestOrderCost} predicates.  Note that we only
generate a winner plan after we have fully explored a branch; the \ol{Pos ==
f\_sizeof(GroupID)} predicate ensures this constraint.  The need for this
explicit constraint is due to the lack of stratification support in P2.

Rule~\ol{bb14} serves as a feedback loop to the search strategy rule~\ol{bb6},
which moves the \ol{branch} position forward by one after fully exploring the
current branch.  A branch is fully explored when winners have been derived
along both child branches.  Rule~\ol{bb14} detects this case with the
\ol{f\_isChildBranch(...)} function, which uses the $PredList$ and (branch)
...
branch bound with the cost given in the \ol{bestGroupCost} predicate, relative
to the parent branch identifier.  We note that the rule~\ol{bb9} is not
predicated on the \ol{winner} predicate relative to the parent branch so it can
be used to provide the latest best cost value.  Furthermore, a rule, similar to
\ol{bb14}, considers cost bounds from \ol{bestOrderCost} by ensuring the lowest
