...
semina\"{\i}ve evaluations derive \ol{path} tuples of length $k$ from {\bf all}
\ol{path} tuples of length $k-1$, and therefore it will not discover a path of
length $j > k$ with a lower cost~\footnote{This assumes that the cost of a
\ol{path} tuple is proportional to it's length i.e., the triangle inequality.}.
