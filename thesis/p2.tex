...
implement and deploy {\em overlay} networks by allowing specifications in a
...
and such a tuple is included in the \ol{path} IDB relation.  The query
predicate, shown at the bottom of Figure~\ref{ch:p2:fig:datalogPath}, asks for
all paths that start at ``node1.'' The \ol{path} tuples that begin with
``node1'' and end at ``node2'' and ``node3'' (via ``node2'') both meet this
query constraint.
...
\begin{boxedminipage}{\linewidth}
    \begin{algorithmic}[1]
      \STATE Empty all \ol{path} facts
      \STATE /* Base case: initialize \ol{path} */
...
Figure~\ref{ch:p2:fig:datalogPath}.  It maintains a $\Delta$\ol{path} reference to
the set of new tuples added to the \ol{path} relation on the previous
round.  In the first round, the algorithm uses rule~\ol{r1} to derive the
initial set of \ol{path} tuples from the EDB \ol{link} relation.  Subsequent
rounds are carried out in the \ol{while} loop until $\Delta$\ol{path} is empty.
The body of the loop contains the following three steps.
\begin{enumerate} \ssp 
  ...
  \item Add the new derivation in
    $\Delta$\ol{path} to the \ol{path} relation.  
\end{enumerate}
...
round, and that those new deductions are added to the \ol{path} relation.
...
terminate under safety constraints.  A Datalog program is said to be at {\em
fixed point} when no further deductions can be made relative to the current EDB
and IDB tuples.  The result derived at fixed point is a model for the Datalog
program.  Given a model~$m$ and a Datalog program~$p$, we say $m$ is a minimal
model if and only if no proper subset of $m$ is a model for $p$.  In the
absence of negated subgoals, a Datalog program has one and only one minimal
model.  A logic program with negation may have more than one minimal model;
however, if the program is stratified, then once again there is a unique
minimal model.~\cite{ullmanbook}
...
evaluate the rules in Figure~\ref{ch:p2:fig:negation} (using say
Figure~\ref{ch:p2:fig:seminaive}) then we could end up with \ol{path} tuples
that cross detours.  To see this, lets assume that we start by evaluating
rule~\ol{r4} with the initial facts in the \ol{link} relation.  The execution
plan for the negated \ol{detour} is similar to an anti-join operation, where
tuples from \ol{link} relation pass if they do not already exist in the current
\ol{detour} relation.  Since we have not yet evaluated rule~\ol{r5}, all
\ol{link} tuples pass the anti-join, and produce a set of \ol{path} deductions
in rule~\ol{r4}.  Subsequently evaluating rule~\ol{r5} would give us our
\ol{detour} tuples, but this would be too late in the sense that we have
...
stratified Datalog.  Before we get to that definition, we first review how the
...
Figure~\ref{ch:p2:fig:seminaive} but with a slight twist -- we sort the IDB
predicates by their assigned stratum, and follow this order in the loop.  This
...
the rule body, before we can evaluate the aggregation in the rule head.
...
selects the actual minimum path based on the minimum cost metric.
...
that order) until no such paths exist.  These derivations are monotonic because
we a performing a {\em min} aggregation of a {\em sum} over non-negative
integers.  As a result, rule evaluations derive \ol{path} tuples of length $k$
before path tuples of length $j > k$.
...
Chapter~\ref{ch:opt} perform a {\em min} aggregation on the cost metric of a
...
extra predicate to a query plan can only increase its cost ({\em principle of
...
that updates the database.  These events a queued in the first phase of the
...
high-level view of this architecture, which contains three queuing elements.
The \ol{event} queue represents the primary input queue, which contains the
current snapshot of tuples that the system uses to drive the processing.  The
\ol{localmem} secondary queue feeds the main event queue with tuples when none
currently exist in it.  Tuples in the \ol{localmem} queue represent
side-affecting events (i.e., insert and delete) to local memory relations.  P2
evaluates this queue in a tuple at a time fashion, where a single tuple is
dequeued and executed in a ``dataflow fixpoint.''
...
the this queue, which continues to drive rule deductions until no zero-lifetime
...
are (silently) queued while the driver then continues to process tuples solely
from the \ol{event} queue until it is again empty.  At this point, P2 declares
a dataflow fixpoint, which triggers a flush of the local~\footnote{Write
actions from the network input are buffered and applied at the end of a
``global fixpoint.''} ``insertion'' action writes from the queue, generating
some number of new event tuples that are added to the \ol{localmem} queue.  If
no ``insertion'' action tuples exist then a flush of the ``deletion'' action
tuples occurs, and the corresponding deletion events are added (directly) to
...
are generated by the system clock and network component, while {\em conditions}
are checked via dataflow processing elements, and {\em actions} initiate
outbound network messages and updates to the database.  All actions are
performed by the evaluator in the final (i.e., third) phase.  A driver loop
...
This sole tuple is the input to the ``current'' Datalog fixed point.  Next, we
