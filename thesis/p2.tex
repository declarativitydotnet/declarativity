...
The \ol{link} fact tuples are used in the evaluation of rule~\ol{R1} to derive
...
Subsequent to that, rule~\ol{R2} performs a transitive closure over any
\ol{link} {\bf and} \ol{path} tuples.  The rule reads ``if there is a link from
$X$ to $Z$ at cost $C$, and there is a \ol{path} from $Z$ to $Y$ over $P2$ at
...
criterion, and will be added to the IDB \ol{path} relation.  A final query
predicate is also present at the bottom of Figure~\ref{ch:p2:fig:datalogPath}
that asks for all paths that start from ``localhost:10000.'' The \ol{path}
...
specify this complete relation prior to evaluation, since \ol{detour}
is an IDB predicate.

If we were to evaluate the rules in Figure~\ref{ch:p2:fig:negation} in an
ad-hoc fashion then we could end up with \ol{path} tuples that cross detours.
...
pass if they do not already exist in the current \ol{detour} relation.  Since
we have not evaluated rule~\ol{R5} yet, all tuples in \ol{link} will pass the
anti-join case and produce a set of \ol{path} deductions in rule~\ol{R4}.
Subsequently evaluating rule~\ol{R5} would finally give us our \ol{detour}
facts, but this would be too late in the sense that we have already made
incorrect deductions.

The reader may realize that by evaluating rule~\ol{R5} first would get us the
...
deriving all paths of length $1$ using rule~\ol{R1}, then using rule~\ol{R2}
...
data (i.e., a \ol{path} tuple) that disputes and earlier deduction. That is,
the naive and seminaive evaluations derive a \ol{path} tuples of length $k$ from
\ol{path} tuples using \ol{path} tuples of length $k-1$~\footnote{This assumes
that the cost of a \ol{path} tuple is proportional to it's length.}. 
...
P2 consist of a dataflow of processing elements, queues, and a single driver
...
\ol{event} queue is the primary input queue that contains the tuples that the
system is currently processing.  The \ol{net in} and \ol{main mem} secondary
...
the \ol{main mem} queue.  If the tuple is remote, it is added to the \ol{net
out} queue.  The third output queue is the \ol{event} queue.  Local output
...
us to drain the \ol{net in} queue. Buried inside the dataflow architecture of
P2 lies another fixpoint-like marker. A P2 dataflow fixpoint occurs when the
\ol{event} queue has been drained, after processing at least one tuple. We describe
this by example.

Assume a single tuple in the \ol{net in} input queue, and all other queues are
empty.  The driver will execute a ``pull-push'' element on the input of the
\ol{event} queue that will dequeue the tuple in the \ol{net in} queue and add
it to the \ol{event} queue.  In an iterative loop, the driver will dequeue a
single tuple from the \ol{event} queue and execute it in the processing
...
into the \ol{event} queue.  The driver loop continues to process \ol{event}
tuples until the \ol{event} queue is again empty.  At that point, it declares a
dataflow fixpoint, which triggers a flush of the ``insertion'' writes from the
\ol{main mem} queue.  If no ``insertion'' tuples exist then a flush of the
...
\ol{net in} input tuple, the system declares a global fixpoint, which is
semantically equivalent to a Datalog fixpoint.  On a global fixpoint, the
driver loop will flush all tuples in the \ol{net out} queue, triggering a
transfer of those tuples to the corresponding P2 instance.  The driver loop
returns to the \ol{net in} queue for the next tuple to process.
...
{\emph floating} EDB, that consists of the single tuple at the head of the \ol{net in}
...
a relevant (insert/delete) \ol{event} is enqueued into the \ol{main mem} queue.

The action for a remote output tuple is simply to enqueue it on the \ol{net
out} queue.  When this queue is eventually flushed, it will be sent out over
