...
{\bf path} in rules~\ol{R1} and \ol{R2}) are part of the IDB, by definition.
...
\ol{link} fact tuples are used in the evaluation of rule~\ol{R1} to derive an
initial set of \ol{path} tuples.  The rule reads ``if there exists a \ol{link}
from $X$ to $Y$ at cost $C$, then there exists a \ol{path} from $X$ to $Y$
...
\datalogspace {\bf link}(X, Y, C). \\
\end{minipage}
Rule~\ol{R3} defines an infinite number of \ol{path} tuples since we can
...
specify this complete relation prior to evaluation, since rule~\ol{R5}
