...
\ol{contains(X, P2) == false} avoids cyclic paths, ensuring a finite result and
...
solution, for example by simply leaving out the \ol{contains(X,~P2)~==~false}
predicate in rule~\ol{r2}} condition for obtaining a finite (IDB) solution from
...
Figure~\ref{ch:p2:fig:datalogPath}.  Let $\Delta$\ol{path} be a reference to
the set of new tuples added to the \ol{path} relation in the previous round.
In the first round, line~$1$ of the algorithm uses rule~\ol{r1} to derive the initial
...
infinite.  Moreover, we cannot specify the complete \ol{detour} relation prior
to evaluation, since it is an IDB predicate (due to rule~\ol{r5}).  If we were
to simply evaluate the rules in Figure~\ref{ch:p2:fig:negation} (using for
example the semina\"{\i}ve algorithm) then we could end up with \ol{path}
...
operation, where tuples from \ol{link} relation pass if they do not already
exist in the current \ol{detour} relation.  Since we have not yet evaluated
rule~\ol{r5}, all \ol{link} tuples pass the anti-join, and produce a set of
\ol{path} deductions in rule~\ol{r4}.  Subsequently evaluating rule~\ol{r5}
would give us our \ol{detour} tuples, but this would be too late in the sense
that we have already made incorrect deductions, and cannot take them
back.~\footnote{Recall: Datalog is a monotonic language
(Chapter~\ref{ch:p2:sec:fixpoint}).}
...
\end{enumerate}
From Figure~\ref{ch:p2:fig:negation}, rule~\ol{r4} forms the
$path~\rightarrow~link$ and negated:~$path~\rightarrow~detour$ edge
dependencies, while rule~\ol{r5} supplies the $detour~\rightarrow~link$ edge
dependency.

The stratum of an IDB predicate $p$ is defined to be the largest number of
negations ($\neg$) along any path involving predicate $p$.  The dependency
graph in Figure~\ref{ch:p2:fig:dependency} places predicates \ol{detour} and
...
stratum if it is included in a cyclic path crosses a negated (subgoal) edge.
...
order ensures that if the program is stratified then any negated subgoal (i.e.,
