...
my $nextSeed = $opt_s;
my $firstLine = "tests/runSymphony 0 $opt_l $nextSeed $firstGateway $opt_D $opt_N >& $firstLog &";
if (!defined $opt_g) {
    print "$firstLine\n";
    system($firstLine);
}
...
while($remainingNodes > 0) {
	...
	my $nextLine = "tests/runSymphony 0 $opt_l $nextSeed $nextNodeAddress $opt_D $opt_N $nextGatewayAddress >& $nextLog&";
	print "$nextLine\n";
	system($nextLine);
	$currentGateway++;
	$nextNode++;
