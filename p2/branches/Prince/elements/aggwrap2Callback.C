// -*- c-basic-offset: 2; related-file-name: "aggwrap2Callback.h" -*-
/*
 * @(#)$Id$
 * 
...
DEFINE_ELEMENT_INITS(Aggwrap2Callback, "Aggwrap2Callback");

Aggwrap2Callback::Aggwrap2Callback(std::string n,
 ...
 */
Aggwrap2Callback::Aggwrap2Callback(TuplePtr args)
...
Aggwrap2Callback::~Aggwrap2Callback()
{
...
int
Aggwrap2Callback::push(int port, TuplePtr t, b_cbv cb)
{
  ...
  case 0:
    ...
    case IDLE:  // Waiting
      ...
      agg_init();
      inner_accepting =
        output(1)->push(t, boost::bind(&Aggwrap2Callback::int_push_cb, this));
...
void
Aggwrap2Callback::comp_cb(int jnum)
{
  TRACE_FUNCTION;
...
Aggwrap2Callback::get_comp_cb()
{
  if (_aggState != CONFIG) {
    ...
    return boost::bind(&Aggwrap2Callback::comp_cb, this, _numJoins++);
  }
}


void
Aggwrap2Callback::agg_init() {
  TRACE_FUNCTION;
  _aggState = BUSY;
...
void
Aggwrap2Callback::agg_accum(TuplePtr t) {
...
void
Aggwrap2Callback::registerGroupbyField(uint32_t outerField)
{ 
  if (_aggState != CONFIG) {
    // Can't give out completion callbacks after configuration has finished.
    ELEM_ERROR("Received a join completion callback request while in state "
