...
#ifndef __VAL_VAR_H__
#define __VAL_VAR_H__

#include "value.h"
#include "oper.h"

class Val_Var : public Value {

public:  
  
  // Required fields for all concrete types.
  // The type name
  const Value::TypeCode typeCode() const { return Value::VAR; };
  ...
  // Constructor
  Val_Var(string str) : s(str) {};
  virtual ~Val_Var() {};

  // Factory
  static ValuePtr mk(string s) { ValuePtr p(new Val_Var(s)); return p; };
