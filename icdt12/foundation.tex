...
locally stratified semantics, and they're only equivalent for programs that have a single stable model.  for example, "p if not q; q if not p" has 2 stable models but no locally stratified model, right?  dang it, now I need to go read that gelfond & livshits paper.}
