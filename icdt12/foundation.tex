...
\begin{example}
Take the following \lang program with input schema $\{\dedalus{q}\}$.  Assume the EDB instance is $\{\dedalus{node(n1), q(n1,0), q(n1,1)}\}$.
...
For Example~\ref{ex:unimportant} with $\oschema = \{\dedalus{concurrent()}\}$, there are two ultimate models: $\{\}$ and $\{concurrent()\}$
\end{example}
