...
\dedalus{c\sub{1}}, \ldots, \dedalus{c\sub{m}} are relations in $\schema$, and
...
\jmh{Following on to the question above about \dedalus{<}, shouldn't we use
  obligatory boilerplate about Horn clauses and finite Herbrand universes?} The
head of a rule \dedalus{p} may not be \dedalus{<}.

Note that we maintain the usual safety
...
appear in a positive atom~\cite{ullmanbook}.  Also, any variable symbol that appears in \od{W} must appear in some \dedalus{\od{X\sub{1}}, \ldots, \od{X\sub{n}}}.
Furthermore, if \dedalus{b\sub{i}} (resp.\ \dedalus{c\sub{i}}) is \dedalus{<},
then any variable that appears in \dedalus{X\sub{i}} (resp.\ \dedalus{Y\sub{i}})
must also appear in \dedalus{X\sub{j}} for some $b_j \neq \dedalus{<}$. That is,
...
spatio-temporal schema $\stschema$.  We will see in Section~\ref{sec:semantics}
