...
A {\em relation instance} for relation \dedalus{r} is a \jmh{finite} set of facts for
...
infinite set of \dedalus{<} facts that encode a total ordering over $\univ$.  \jmh{Should we bother to constrain \dedalus{<} to the finite domain of symbols in the relation instances?}
...
constants from $\univ$ or variable symbols.  The {\em atom} to the left of the $\leftarrow$ is called the {\em head} of the rule, and the conjunction of atoms to the right is called the rule's {\em body}.  \jmh{Following on to the question above about \dedalus{<}, shouldn't we use obligatory boilerplate about Horn clauses and finite Herbrand universes?}

The head of a rule \dedalus{p} may not be \dedalus{<}.  Note that we maintain the usual safety
restrictions of Datalog rules: \jmh{does this colon introduce the entire rest of this paragraph?  If so you need to punctuate a bit differently.  The remainder of the paragraph is pretty boring it'd be nice to know I can skip it as ``yeah yeah, safety''} any variable symbol \dedalus{V} that appears in
...
The latter two kinds of rules are collectively called {\em temporal} rules.  \jmh{Give intuition.  Note the unification of the spatial and temporal attributes in all body predicates; this restriction corresponds intuitively to considering deductions that can be evaluated at a single node at a single point in time.  ALSO NOTE SUCC AND CHOICE.}
...
All cycles involving a negated edge in a \lang program's PDG must also involve a temporal edge.  \jmh{Why?  Sez Who?}
...
Recall that \dedalus{choice} is only used in asynchronous rules.  A \lang program without \dedalus{choice} is {\em locally stratified}~\cite{local-strat}, because of restrictions on negation \jmh{elaborate?}; thus, it is natural to use the locally stratified semantics to define the mapping for a \lang program of this kind.  Sacc\`{a} and Zaniolo~\cite{sacca-zaniolo} propose the {\em stable model semantics} as a natural interpretation of \dedalus{choice}.  We do not review the stable model semantics here.  \jmh{So screw you, lazy reader!  I.e. this sounds rude.  Can we find a way to give them what they need to proceed?}  Defining the mapping using the stable model semantics, it is not hard to see that each stable model is a spatio-temporal database instance that defines a possible function for \dedalus{choice} that obeys the causality constraint; every possible function that obeys the causality constraint defines a stable model.  \jmh{Wow, this was a jump.  First you assume we know stable models and say "it is not hard to see" something.  Then you make reference to causality constraints without defining them even by reference.  Yuck!  What is your point in this paragraph?  Maybe state the goal of the argument at the top of the para, and if you need to make reference to esoterica you will have motivated it up front.}  It is known that the stable model semantics coincides with the locally stratified semantics~\cite{stable-model} for locally stratified programs.
...
The stable models \jmh{of \dedalus{q}??} are exactly $\{ \, \dedalus{q(n1,i,0), q(n1,j,1)} \, | \, (i,j) \in (t_1, t_2) \, \land \, (t_1, t_2) \in \mathcal{N}
...
First, a program with even one asynchronous rule may have uncountably many stable models, and not all of these may be meaningfully different. \jmh{meaningful in what sense??} Second, a stable model of a \lang program may itself be infinite.  \jmh{So what?}  We address both concerns in our definition of an {\em ultimate model}.
