...
There is a usage of negation that is both intuitive and corresponds to common
distributed systems practices: negation is not applied until the negated
relation is ``done'' being computed.

Given a \lang program $S$ with guarded asynchrony whose PDG does not contain any
cycles through negation, the \plang program of $S$, denoted $\mathcal{P}(S)$, is
...
\plang maintains the desirable properties of \slang: it is both confluent and PTIME.  We first argue that the rules for computing \dedalus{p_done{}} have the desired effect, explain why this implies \plang is temporally inflationary, and show how this implies that a \plang program can be decomposed into a sequence of \slang programs; the PTIME and confluence results are then simple corollaries.
