...
First, we define the notion of a {\em predicate dependency graph} (PDG).  The PDG of a \lang program $P$ with spatio-temporal schema $\stschema$ is a directed graph with one node per relation; each node $i$ has a label $L(i)$.  If node $i$ represents relation \dedalus{p}, then $L(i) = \dedalus{p}$.  There is an edge from the node with label $\dedalus{q}$ to the node with label $\dedalus{p}$ if relation \dedalus{p} appears in the head of a rule with \dedalus{q} in its body.  If some rule with \dedalus{p} in the head and \dedalus{q} in the body is asynchronous (resp.\ inductive), then the edge is said to be {\em asynchronous} (resp.\ {\em inductive}).  If some rule with \dedalus{p} in the head has \dedalus{$\lnot$q} in its body, then the edge is said to be {\em negated}.  Collectively, asynchronous and inductive edges are referred to as {\em temporal edges}.  The PDG does not contain nodes for the \dedalus{node}, \dedalus{succ}, or \dedalus{time} relations, or any relation introduced in the causality or \dedalus{choice} rewrites.

\plang is the language of \lang programs with guarded asynchrony whose PDG does not contain any cycles through negation.  As is standard, a \plang program can be partitioned into {\em strata}.  The {\em stratum} of a relation \dedalus{r} is the largest number of negated edges on any path from \dedalus{r}.
...
While the model-theoretic semantics of \plang are clear, \plang's negation semantics are different than those of \lang.  Thus, we cannot directly apply the correspondence to a distributed operational semantics in~\ref{ameloot-operational}.  Fortunately, we can rewrite any \plang program to a \lang program.

Intuitively, the rewrite involves determining when the 
...
IDB relations, defining \dedalus{p_done()} takes some work.  Intuitively, \dedalus{p_done()} for $\dedalus{p} \in L(i)$ directly depends on \dedalus{r_done()} for any \dedalus{r} in the body of a rule with \dedalus{p} in the head.  Additionally, asynchronous rules take some care---while deductively-defined relations are done in the same timestamp as all relations they depend on, there may be arbitrary delay before asynchronously-defined relations are done.
...
For non-async-recursive nodes, we can compute a \dedalus{done} fact for each
rule, then collate these into \dedalus{done} facts for each relation.  We
handle deductive and asynchronous rules separately. The \dedalus{done} fact for a deductive rule is true when all of the relations in the body of the
rule are henceforth unchanging. The \dedalus{done} fact for an asynchronous rule is henceforth true at some local timestamp after all facts derived in the head relation are true at their respective locations.   We assume guarded asynchrony applies to
...
%uq\sub{\phi}(\od{W}, \od{X}, 0) <- min(\od{X}), $$\lnot$\phi(\od{W}, \od{X})$.
...
ordinary column in the \dedalus{p\sub{j}_to_send} relation (due to the absence
of \dedalus{#}).  Note that because this first rule is a deductive rule, as well as the only rule defining \dedalus{p\sub{j}_to_send}, the \dedalus{p\sub{j}_to_send} relation is done at the same time as the body relations of the first rule (this is shown in the second rule).  The third rule copies messages to the correct destination location specifier, while including the location specifier of the source (\dedalus{L}).  The fourth derives acknowledgements at the source's location specifier.  The fifth rule (at the source) derives a \dedalus{r\sub{j}_done_node} fact at a node when the source has an \dedalus{p\sub{j}_ack} for each \dedalus{p\sub{j}_send}.  Note that the causality constraint ensures that the timestamp chosen for each \dedalus{r\sub{j}_done_node} message is greater than any timestamp before the stable model satisfies the body of the rule.  The final rule (at the destination) asserts that rule $j$ is done once \dedalus{r\sub{j}_done_node} has been received from all nodes---intuitively, the rule is done when all messages from all nodes have been received.
...
\drule{p\sub{\phi}_min(\od{W},\od{X})}
      {p(\od{W},\od{X}), $\lnot$p\sub{\phi}_succ(\od{W},\od{_},\od{X}), p\sub{\phi}_succ_done()}
\drule{p\sub{\phi}_max(\od{W},\od{X})}
      {p(\od{W},\od{X}), $\lnot$p\sub{\phi}_succ(\od{W},\od{X},\od{_}), p\sub{\phi}_succ_done()}
...
The first four rules above compute a total order over \dedalus{p\sub{\phi}}.  The final four rules iterate over the total order of \dedalus{p\sub{\phi}}, and checking each \dedalus{p\sub{\phi}} to see if \dedalus{q} also holds.  If \dedalus{q} does not hold for any \dedalus{p}, iteration will cease.  However, if \dedalus{q} holds for all \dedalus{p} then \dedalus{forall\sub{\phi}} is true.

We additionally need to add a rule for the vacuous case of the universal quantification.  In general, we cannot write \dedalus{forall\sub{\phi}(\od{W}) $\leftarrow$ $\lnot$p(\od{W},\od{_}), p_done().}, because the variables in \od{W} do not obey our safety restrictions.  Thus, for every rule $r$ that contains \dedalus{\(\forall \od{X} . \phi(\od{W},\od{X})\)} in its body, we must duplicate $r$, replacing the $\forall$ clause with the atom \dedalus{$\lnot$p(\od{W}, \od{_})}.
...
Once a node has received a \dedalus{vote_1\sub{i}} vote solicitation, it also begins keeping track of whether it has sent any messages in the async recursive component; this information is erased if another \dedalus{vote_1\sub{i}} solicitation is received.  The causality constraint implies that \dedalus{$\lnot$all_ack\sub{i}()} is true if a message is sent, because messages cannot be instantly acknowledged.
...
\drule{start_round_2\sub{i}()}
      {$\lnot$not_all_recv_1\sub{i}(), $\lnot$not_all_comp_1\sub{i}()}
\end{Drules}
...
We first argue that the rules for computing \dedalus{p_done{}} have the desired effect.
...
%In the other direction, it is clear that we can encode any stratified Datalog program in% \plang using deductive rules: the ultimate model coincides wieh the perfect model of th%e Datalog program.                                                                   %\end{proof}  
...
\begin{Drules}
  ...
  \drule{refers_to_done(M)}
        {\(\left(\forall \dedalus{N}.\dedalus{node(N)} \Rightarrow \dedalus{refers_to_done_node(M, N)}\right)\)}
  \drule{reach_done()}
        {refers_to_done(), \(\left(\forall \dedalus{N}.\dedalus{node(N)} \Rightarrow \dedalus{local_ptr_edb_done(N)}\right)\)}
\end{Drules}
\end{example}
