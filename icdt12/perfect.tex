...
Given a \lang program $S$ with guarded asynchrony, whose PDG does not contain any cycles through negation, the \plang program of $S$, denoted $\mathcal{P}(S)$, is the \lang program obtained by adding \dedalus{p_done()} to the body of any rule in $S$ that contains a \dedalus{!p} atom, and adding rules to define \dedalus{p_done()} as described in subsequent sections.

We will see the \dedalus{p_done()} has the
...
We show that \plang maintains the nice properties of \slang: it is both confluent PTIME.  We first argue that the rules for computing \dedalus{p_done{}} have the desired effect, explain why this implies \plang is temporally inflationary, and show how this implies that a \plang program can be decomposed into a sequence of \slang programs; the PTIME and confluence results are then simple corollaries.
