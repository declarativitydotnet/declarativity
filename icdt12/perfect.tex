...
Formally, a \plang program is a \lang program $P$ with guarded asynchrony, whose PDG does not contain any cycles through negation.  All rules in $P$ with a \dedalus{!p()} body atom must also have a \dedalus{p_done()} atom in the body.  A \plang program must also contain rules that define \dedalus{p_done()}, for all relations \dedalus{p}, as described in subsequent sections; however, it will become clear that these rules can be automatically added, given a \plang program with no rules for \dedalus{p_done()}.

The relation \dedalus{p_done()} has the
...
For EDB relations \dedalus{p}, $P$ must contain the rule \dedalus{p_done().}~\footnote{This is actually a rule.  Consider the unsugared form: \dedalus{p_done(L,T) \(\leftarrow\) node(L), time(T).}}  For
...
For non-async-recursive nodes, we can compute a \dedalus{done} relation for each rule, then collate these into \dedalus{done} relations for each relation.  We seperately handle atemporal and asynchronous rules: an atemporal rule is done at the same time as its body relations; an asynchronous rule is done, intuitively, when ``all messages have been acknowledged.''
...
\drule{p(\od{X})}
      {p\sub{j}_send(_,\od{X})}
\end{Drules}

The first rule stores messages to be sent at the body (source's) location specifier.  This is necessary so the source can check whether all messages have been acknowledged.  The original destination location specifier is stored as an ordinary column in the \dedalus{p\sub{j}_to_send} relation (due to the absence of \dedalus{#}).  Note that because this first rule is an atemporal rule, as well as the only rule defining \dedalus{p\sub{j}_to_send}, the \dedalus{p\sub{j}_to_send} relation is done at the same time as the body relations of the first rule.  The second rule copies messages to the correct destination location specifier, while including the location specifier of the source (\dedalus{L}).  The third derives acknowledgements at the source's location specifier.  The fourth rule (at the source) derives a \dedalus{r\sub{j}_done_node} fact at a node when the source has an \dedalus{p\sub{j}_ack} for each \dedalus{p\sub{j}_send}.  The fifth rule (at the destination) asserts that the rule is done once \dedalus{r\sub{j}_done_node} has been received from all nodes.  The final rule copies \dedalus{p\sub{j}_send} into \dedalus{p}, as the original program references \dedalus{p}.
...
\paa{this edit makes the statement too weak: these rules are automatically synthesized. if you submit the original GC example qua dedalus\_p program, this program is run.}
