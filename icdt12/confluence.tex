...
%Let $\{O_1, \ldots, O_k\} \subset \{1, \ldots n\}$ be the output nodes; we add the rule: \dedalus{done() <- done\sub{O_1}(), \ldots, done\sub{O_k}().}.  The \dedalus{done()} relation is true when the contents of all output relations are sealed.
...
\begin{Dedalus}
done\sub{i}() <- r\sub{1}_done(), \ldots, r\sub{i_p}_done().
\end{Dedalus}

Assume that the nodes in the collapsed PDG connected via incoming edges to node $i$ are numbered $1, \ldots, i_q$.

For each rule \todo{introduce some symbol like $P$ for the program} $1 \leq j \leq i_p$ with head relation \dedalus{p}, if $j$ is asynchronous, add the rule:

\begin{Dedalus}
r\sub{j}_done() <- n\sub{1}_done(), \ldots, n\sub{i_q}_done().
\end{Dedalus}

else if $j$ is inductive, add the rule:

\begin{Dedalus}
r\sub{j}_done()@next <- n\sub{1}_done(), \ldots, n\sub{i_q}_done().
\end{Dedalus}
...
\todo{DOESNT OBEY THE NEGATION CONVENTION OF DEDALUS-C UNFORTUNATELY so we need to rewrite this using an order which is going to blow up the number of rules by a ton}

%p\sub{j}_to_send_min(N,\od{X}) <- p\sub{j}_to_send(N,\od{X}), !p\sub{j}_to_send_succ(N,\od{_},\od{X}).                                                                         
%p\sub{j}_to_send_max(N,\od{X}) <- p\sub{j}_to_send(N,\od{X}), !p\sub{j}_to_send_succ(N,\od{X},\od{_}).                                                                         
%p\sub{j}_to_send_succ{N,\od{X},\od{Y}) <- p\sub{j}_to_send(N,\od{X}), p\sub{j}_to_send(N,\od{Y}), !p\sub{j}_to_send_not_succ(N,\od{X},\od{Y}).                                 
%p\sub{j}_to_send_not_succ{N,\od{X},\od{Y}) <- p\sub{j}_to_send(N,\od{X}), p\sub{j}_to_send(N,\od{Y}), p\sub{j}_to_send(N,\od{Z}), \od{X} < \od{Z}, \od{Z} < \od{Y}. 
...
p\sub{j}_unacknowledged() <- p\sub{j}_to_send(N,\od{X\sub{0}}), !p\sub{j}_ack(N,\od{X\sub{0}}).
r\sub{j}_done() <- n\sub{1}_done(), \ldots, n\sub{i_q}_done(),
!p\sub{j}_unacknowledged().
\end{Dedalus}

\subsubsection{Inductive Recursive Nodes}

For each inductive recursive node $i$, assume that the relations in $L(i)$ are numbered $1,\ldots,i_p$.  Add the rule:

\begin{Dedalus}
n\sub{i}_done() <- !not_n\sub{j}_fixpoint().
\end{Dedalus}

For each $1 \leq j \leq i_p $, add the rule:

\todo{Again, doesn't obey the negation convention of Dedalus-C, so we need to rewrite using order}

\begin{Dedalus}
p\sub{j}_prev(\od{X})@next <- p\sub{j}(\od{X}).
not_n\sub{j}_fixpoint() <- p(\od{X}), !p_prev(\od{X}).
\end{Dedalus}
