...
However, it is often desirable to ensure that a program specification on a fixed input has a single, deterministic output, regardless of non-determinism in the order of message arrivals (captured in \lang by \dedalus{choice}).  
...
In particular, we can assume that the \dedalus{succ} of the body timestamp is always chosen:
