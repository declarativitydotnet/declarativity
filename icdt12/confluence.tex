...
outputs (multiple ultimate models) or diverge (no ultimate model). \nrc{This is
  wrong: a program can easily diverge but have an ultimate model (e.g., flipflop
  program + some trivially-persistent state).}  In practice, such programs are rarely 
desired and indeed often indicate a bug in the specification.\nrc{Dubious:
  ...
  formulation. We should probably concede that confluence, as formulated, is not
...
At each timestep, we check whether the current variable assignment is true.  If it is true, then \dedalus{formula_true()} is true at that timestep.  We omit these rules.
...
element in \dedalus{b_edb()} according to the order \dedalus{<}.
...
For EDB relations \dedalus{p}, $P$ must contain a \dedalus{p_done()} fact.  For
...
\begin{Drules}
  \drule{p(#N,\od{X\sub{0}})@async}
        {b\sub{1}(#L,\od{X\sub{1}}), \ldots, b\sub{n}(#L,\od{X\sub{n}}), c\sub{1}(#L,\od{X\sub{1}}), \ldots, c\sub{m}(#L,\od{X\sub{m}})}
\end{Drules}
...
\begin{Drules}
\drule{p\sub{j}_to_send(N,\od{X\sub{0}})}
      {b\sub{1}(#L,\od{X\sub{1}}), \ldots, b\sub{n}(#L,\od{X\sub{n}}), c\sub{1}(#L,\od{X\sub{1}}), \ldots, c\sub{m}(#L,\od{X\sub{m}})}
