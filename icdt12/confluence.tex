...
For EDB relations \dedalus{p}, $P$ must contain a \dedalus{p\_done()} fact.  For
IDB relations, defining \dedalus{p\_done()} takes some work.  A rule defining
\dedalus{p\_done()} for IDB relation \dedalus{p} may use \dedalus{q\_done()} in
...
\dedalus{p} are numbered $1, \ldots, i_p$.  The rule for \dedalus{p\_done()}
