...
EDB relation \dedalus{local_ptr_edb}. \nrc{Problem with this example:
  ...
  async facts will be delivered yet. Also, claim about inflationary could be
  tightened up.}
...
First we will see that \slang programs are {\em inflationary} -- i.e., if a stable model of a \slang instance contains a spatio-temporal fact \dedalus{f@t}, then it also contains \dedalus{f@t+1} (and thus the ultimate model contains \dedalus{f}).
...
Any instance of this program has a single ultimate model in which \dedalus{b()} contains the highest element in \dedalus{b_edb()} according to the order \dedalus{<}.  Thus it is confluent, but the program uses IDB negation and does not have guarded asynchrony.

Not only are \slang programs confluent, but they also capture exactly PTIME.  We will prove this by showing an equivalence to Datalog programs, which are known to capture exactly PTIME over ordered structures (Theorem 14.13 in~\cite{immerman-book}).
