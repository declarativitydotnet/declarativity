...
However, it is typically desirable to ensure that a program specification on a fixed input has a single, deterministic output, regardless of non-determinism in the order of message arrivals (captured in Dedalus by \dedalus{choice}).  
...
transitive closure of \dedalus{points\_to} \jmh{don't you mean reach?} is fully determined (i.e., after all
...
\end{lemma}
\jmh{I vote to move the proof to the appendix, and give a hint here of how it's done (via QBF).}
...
We show how to write the PSPACE-complete Quantified Boolean Formula (QBF) problem \jmh{Citation needed, even if only to Garey/Johnson} in \lang. Since \lang is closed under first-order reductions, and QBF is PSPACE-complete under first-order reductions, we have that $\text{PSPACE} \subset \lang$.
...
the constant \dedalus{t} for the timestamp. \jmh{This important notation comes rather "by the way" here.  Is there a better spot to introduce it?}
...
contains \dedalus{f@t+1} (and thus the ultimate model contains \dedalus{f}). \jmh{Doesn't inflationary have a meaning in Datalog?  Do we need to coin a term like "temporally inflationary"?}
