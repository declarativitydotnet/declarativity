...
desired and indeed often indicate a bug in the specification.\nrc{Dubious:
  long-running services might well have no ultimate model according to this formulation.}
...
\begin{definition}
A \lang program is {\em confluent} (has a deterministic output) if, for every
instance of the program, it has a unique ultimate model.  A program that is not
confluent is {\em diffluent}.
\end{definition}
...
such programs while retaining the convenience and power of \lang.  We will
present a natural restriction of \lang that allows only confluent programs, and
