...
\begin{abstract}
  ...
  take a model-theoretic approach to analyzing confluence ("eventual consistency") of asynchronous
  distributed programs. 
  We begin with a model-theoretic
  semantics for \lang~\cite{ameloot-operational}, and develop the concept of \emph{ultimate models} to capture the
  non-deterministic eventual outcomes of distributed programs.  After demonstrating the undecidability of checking confluence for \lang programs, we look for restricted sub-languages that guarantee confluence while providing adequate expressivity.  
We observe that a simple semipositive restriction called \slang guarantees confluence while capturing  PTIME, but demonstrate that the limited use of negation in \slang makes certain simple and practical programs very difficult to express.  To remedy this, we introduce \plang, a restriction of
  \lang that allows a natural use of 
  negation in the spirit of stratified negation, but retains the
  confluence of \slang and similarly captures PTIME.  
%\plang is the formal  basis for our applied programming language, Bloom~\cite{bloom}.
