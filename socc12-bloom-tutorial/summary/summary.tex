...
In this tutorial we focus predominantly on monotonic programs, demonstrating that 
it is possible to express
surprisingly complicated systems without recourse to nonmonotonic constructs.
Like side-effects in functional programming languages, however, nonmonotonicity 
cannot always be avoided in practical programs.  To conclude the tutorial,
we will discuss our recent and future work on mitigating the problems that arise when
distributed systems are partly nonmonotonic.  We will discuss the use of CALM analysis to
identify program locations at which coordination can be added or synthesized.  
We will also discuss approaches that tolerate and attempt to correct inconsistencies
using delayed coordination, compensations or ``apologies.''  We will present constructive strategies
