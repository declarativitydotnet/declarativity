...
\item
  ...
  Provide concrete examples of CvRDTs (convergent distributed datatypes)
  including sets, counters and graphs, and illustrate how these data structures
  maintain the guarantee of eventual consistency.
\item
...
\item
  As a running example, implement a family of key-value store implementations
  with various consistency guarantees in Bloom.  Aided by static analysis tools,
  contrast the nondeterministic executions of overwrite-based stores with 
  stores whose values are constrained to be lattices, and hence are convergent
  in either the value or the data item version domain.  Illustrate the use of 
  lattice compositions to implement quorum replication, which can be 
  parameterized to enforce strong or eventual consistency.
\item
