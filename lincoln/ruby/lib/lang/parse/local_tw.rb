...
require 'lib/types/function/aggregate_fn'
...
class OverlogCompiler
  ...
  class VisitAggregate < VisitGeneric
    ...
    def semantic(text,obj)
      super(text,obj)
      t = text.gsub('"',"")
      print_table("primaryExpression", [@@positions["_Universal"], @@current["expression"], @@positions["_Primpos"], '"'+t+'"', "agg_func", "??"])        #@pet.insert(TupleSet.new("function",Tuple.new(@@positions["_Universal"],@@current["expression"],@@positions["_Primpos"],'"'+obj.func.text_value+'"',"agg_func","??")),nil)

      # for Min and Max, type of the Agg result depends on type of the 
      # var inside.  How do we grab that????
      varName = obj.text_value.split("<")[1].delete(">")
      varType = Numeric # WRONG!  COP OUT!!
      otabinsert(@pet, @@positions["_Universal"], @@current["expression"], @@positions["_Primpos"], obj.text_value, "agg_func",AggregateFunction.type(obj.func.text_value, varType))
