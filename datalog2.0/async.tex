...
%~\footnote{Note that the perfect model semantics coincides with the stable model semantics for \lang programs that are both temporally safe and temporally stratified.}  too confusing to add -wrm
...
Finally, certain computational substrates guarantee monotonicity in both timestamps and message ordering---for example, some multiprocessor cache coherency protocols achieve this.  When temporal monotonicity is given, the proof of temporal stratification 
