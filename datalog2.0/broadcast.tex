...
Assume that \dedalus{members} is a persistent relation that contains the
broadcast membership list.  The protocol is straightforward: if a tuple appears
in \dedalus{smessage} (an EDB predicate), then it will be sent to all members (a
multicast).  The interpretation of the non-deterministic choice implied by the
\dedalus{@async} rule indicates that messaging order and delivery (i.e., finite
