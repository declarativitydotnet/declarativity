...
int main(int argc, char** argv)
{
	...
	std::map<uint64_t, byte> exact;
	std::map<uint64_t, byte>::iterator itEx;
	...
	try
	{
		...
		for (size_t i = 1; i <= N; i++)
		{
			...
			if (i % 100 == 0)
			{
				...
				std::multimap<uint64_t, byte> frequent;
				for (itEx = exact.begin(); itEx != exact.end(); itEx++)
					frequent.insert(
						std::pair<uint64_t, byte>(
							(*itEx).second,
							(*itEx).first)
					);

				std::multimap<uint64_t, byte>::iterator it = frequent.end(); it--;

				size_t top = 20;
				while (top > 0)
				{
					byte fU = bfU.getFrequency((*it).second);
					byte fS = bfS.getFrequency((*it).second);
