// -*- c-basic-offset: 2; related-file-name: "GangliaTail.h" -*-
/*
 * @(#)$$
 *
...
// Constructor
GangliaTail::GangliaTail(Stage* myStage)
  : Stage::Processor(myStage),
    _acc("")
{
STAGE_WARN("GangliaTail constructor\n");
...
//		return std::make_pair(TuplePtr(), Stage::DONE);
  ...
  TuplePtr resultTuple =
    Tuple::mk();
  resultTuple->append(GangliaTailTuple);
  resultTuple->append(_locationSpecifier);
  // iterate through tuple fields and append
  for (unsigned int i = 0; i < p->size(); i++) {
	if (i==0) 
	{
		string tempTS = ((*p)[i])->toString();
		tempTS = tempTS.substr(0, 10) + tempTS.substr(11, 3);
		resultTuple->append(Val_Int64::mk(strtoll(tempTS.c_str(), NULL, 0)));
	}
	else
	{
  ...
  /** The input schema for GangliaTail is <tupleName, location specifier, filename>. 
  ...
  */
void
GangliaTail::newInput(TuplePtr inputTuple)
{
...
ValuePtr GangliaTail::GangliaTailTuple = Val_Str::mk(std::string("GangliaTail"));



// This is necessary for the class to register itself with the
// factory.
DEFINE_STAGE_INITS(GangliaTail,"GangliaTail")
