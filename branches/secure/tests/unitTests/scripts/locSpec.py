...
def usage():
        print """
                locSpec.py -E <planner path> -B <olg path>
...
for key,val in opt:
        if key=='-B':
                ...
                sys.exit(0)
try:
        args=[executable_path , '-DLOCALID=\"localhost:11111\"', '-DNEIGHBORID=\"localhost:22222\"', '-DEVENTID=235691', '-o', olg_path +'/locSpec.olg', '-n', 'localhost', '-p', '11111']
...
try:
	args=[executable_path , '-DLOCALID=\"localhost:22222\"', '-DNEIGHBORID=\"localhost:11111\"', '-DEVENTID=235691', '-o', olg_path +'/locSpec.olg', '-n', 'localhost', '-p', '22222']
...
if os.getpid() != pid_11111 and os.getpid() != pid_22222:
        t = threading.Timer(10, kill_pid, [p_11111.stdout, p_22222.stdout, pid_11111, pid_22222])
        t.start()
