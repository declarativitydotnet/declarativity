...
Computing the ultimate model of a program with no asynchronous rules is easy---any \lang program with no asynchronous rules is a $\text{Datalog}_{1S}$ program~\cite{tdd}, whose trivial periodicities can be computed in PSPACE~\cite{tdd}.  With asynchronous rules we have an additional complication: messages may be delayed arbitrarily.  We run iterations of their algorithm, keeping track of which subsets of messages we've seen.  If in some iteration, we do not see a new subset, we have computed the ultimate model.  Since there are only finitely many possible subsets of messages, we will eventually see them all and eventually terminate.

We now prove that our model-theoretic and operational semantics are equivalent.

\begin{lemma}
For any \lang program, the family of ultimate models exactly corresponds to the ultimate models achievable by the operational formalism.
\end{lemma}
\begin{proof}
