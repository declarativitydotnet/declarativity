...
For example, the trivially periodic fact \{\dedalus{p(a,7); p(a,8); p(a,9);} \ldots\} is represented as \dedalus{p(a)}.   We refer to the ultimate model of a program $P$, EDB $E$, and nondeterministic choice set $ND$ as $UM(P, E, ND)$.  Note that it may be the case that $UM(P, E, ND_1) \neq UM(P, E, ND_2)$ if $ND_1 \neq ND_2$.
\end{definition}
...
We will compute each partition of $UM(P, E, ND)$ separately.  It is the case that $UM = $

Any \lang program without asynchronous rules is a $\text{Datalog}_{1S}$ program, whose ultimate model can be computed in polynomial space in the size of the input~\cite{tdd}.  For a given node \dedalus{n}, let $G$ be the number of ways to instantiate all predicates' generic arguments with all constants that appear in the program and $E_{\text{\dedalus{{\scriptsize n}}}}$, and let $c$ be the maximum timestamp of any fact not derived by a rule with body location variable bound to \dedalus{n}.  In this case, $c = 1$ becasue the only such facts are EDB, but this assumption will not hold when we consider asynchronous rules.
