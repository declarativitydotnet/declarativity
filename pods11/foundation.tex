...
We now outline a set of restrictions on \lang that limit the use of special constructs for time and space.
...
The use of \dedalus{successor} and \dedalus{choice} is prohibited in \lang, except in these special usages.
...
we introduce syntax sugar that omits the body temporal attributes.  Since the relationship between the head and body temporal variables is tightly constrained, we omit the head temporal attribute, and the body predicates involving \dedalus{successor}, \dedalus{choice} and \dedalus{time}, and instead introduce a temporal annotation to the head predicate: \dedalus{@next} or \dedalus{@async}.  We also omit the location attribute in local rules, and the \dedalus{node} predicate in all rules.  In Example~\ref{ex:sugared}, we rewrite the above rules using this syntax sugar.
