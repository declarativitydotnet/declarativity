...
\wrm{With asynchronous rules, every node begins an exponential number of phases in the size of their input.  When they receive a new set of input that they haven't seen before, they forget about the current phase, and start a new phase exponential in the new total input size (old input size + new elements they got over the network).  When the exponential phase termintes without a new one killing it and taking over, the node halts.  When all nodes have halted, and no messages are in flight (will happen eventually) we're done.  Takes a finite number of steps.}

\wrm{It is the case that this process emulates some prefix of some stable model.  The concern as to why this may not produce an ultimate model is that we don't process things long enough.  Chomicki proves this ain't so.  Thus, the prefix computed corresponds to a stable model that repeats all of the periodicities (and any further asyncs happen henceforth deterministically).}

\wrm{Either assume bounded message delay or admit failure in order to enforce real-time constraints.  We show how failure is modeled later...}
