...
\begin{definition}
For each stable model, the cycles \wrm{cyclic facts...} of period 1---so-called {\em trivially periodic} cycles---comprise an {\em ultimate model} for the program.  We represent a trivially periodic cycle in the ultimate model finitely by picking a representative, and discarding its timestamp.  For example, the trivially periodic cycle \{\dedalus{p(a,7); p(a,8); p(a,9);} \ldots\} is represented in the ultimate model as \dedalus{p(a)}.  A program may have multiple distinct ultimate models if it has multiple stable models with different trivial periodicities.
\end{definition}
...
Let $G$ be the number of ways to instantiate all predicates' generic arguments with all constants from the generic universe, and let $c$ be the maximum depth of any temporal fact---in this case, $c=1$, because all temporal facts are EDB, and all EDB facts are considered during the first timestamp.  The algorithm runs the program for $2^{G+1} + c$ timestamps and tracks the periodicity of each of the facts---there are polynomially many of these in the size of the data.  Evaluation of a single timestamp is akin to evaluation of a Datalog program, which may take polynomial time~\cite{immerman-ptime}.  After evaluating the Datalog program at time $i$, the IDB facts whose timestamp is $i+1$ are considered the EDB of the Datalog program at time $i+1$.
