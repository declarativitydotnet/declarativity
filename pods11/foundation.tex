...
When we talk about the {\em predicate dependency graph} (PDG) of a \lang program, we assume that it does not include the \dedalus{time} or \dedalus{successor} predicates~\footnote{We also assume that the graph does not include any predicates or rules added by rewriting \dedalus{choice}.}.

By convention, EDB facts are supplied without a time suffix, and are true at all times.
