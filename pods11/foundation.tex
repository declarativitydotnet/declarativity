...
\lang restricts negation by requiring any recursion through negation to include a temporal edge\footnote{The only exception is the rewrite of \dedalus{choice} \jmh{clean up this ref to the use of choice...not clear what you mean by ``the rewrite of choice''}, which involves recursion through negation without any temporal edges.}  Any recursion through \dedalus{count} is disallowed, in order to ensure finiteness of the generic universe.
...
\jmh{line of motivation here for the rest of the paragraph, please.}
For a given node \dedalus{n}, let $G$ be the number of ways to instantiate all predicates' generic arguments with all constants that appear in the program and $E_{\text{\dedalus{{\scriptsize n}}}}$, and let $c$ be the maximum timestamp of any fact not derived by a rule with body location variable bound to \dedalus{n}.  In this case \jmh{which case do you mean?}, $c = 1$ because the only such facts are EDB; this assumption will not hold when we consider asynchronous rules.

The algorithm \jmh{what algorithm?} runs each partition of the program for $2^{G+1} + c$ timestamps and keeps track of the periodicity of each of the facts over the previous $2^G$ timestamps.  Note that $2^G$ is the maximum period of any fact~\cite{tdd}, and there are polynomially many facts.  Evaluation of a single timestamp is akin to evaluation of a Datalog program, which may take polynomial time~\cite{immerman-ptime, vardi-ptime}.  After evaluating the Datalog program at time $i$, the IDB facts whose timestamp is $i+1$, as well as the tuple \dedalus{successor(i+1, i+2)} are considered the EDB of the Datalog program at time $i+1$.
