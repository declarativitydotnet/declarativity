...
Our goal in defining an operational formalism is to map real-world behavior to models of our logic program.  As one might expect, given our definition of the ultimate model, we will compute our ultimate model by performing a distributed analog to computing a partial fixpoint.  \wrm{waiting for guidance from Joe and/or Peter on what which direction we need to prove here...}
We first present an operational model of a distributed system, and explain how it can be modeled in our language.  We then present an algorithm for computing ultimate models, explain how it can be run on our operational formalism, and show that it can compute precisely the set of ultimate models given by the model-theoretic semantics.
...
Intuitively, the operational model is based on $n = |\dedalus{node}|$ Turing Machines, which independently step sequentially through time and communicate via channels with non-deterministic delay.  At each timestep \dedalus{t} they run a datalog fixpoint computation that evaluates $P$ on $projection(E_n, t)$; this takes polynomial time~\cite{immerman-ptime, vardi-ptime}.  At the end of this fixpoint there are three sets of relevant facts: local, synchronous facts that have timestep \dedalus{t+1} and become part of $projection(E_n, t+1)$, local asynchronous facts whose timestep is chosen non-deterministically to be greater than t and become part of later timesteps, and remote asynchronous facts.  The timestamps in this third class of facts are chosen non-deterministically ``at the receiver'' to model delay, in a way that observes traditional causality restrictions~\cite{lamportclocks}.
