...
If predicate \dedalus{p} appears in the head of a rule with \dedalus{q} in its body
...
For example, the trivially periodic fact \{\dedalus{p(a,7); p(a,8); p(a,9);} \ldots\} is represented as \dedalus{p(a)}.  We denote the ultimate model of a program $P$, EDB $E$, and nondeterministic choice set $ND$ as $\mathcal{U}(P, E, ND)$.  Where $\forall ND_1, ND_2, \mathcal{U}(P, E, ND_1) = \mathcal{U}(P, E, ND_2)$, we denote the unique ultimate model for program $P$ with EDB $E$ as $\mathcal{U}(P, E)$.  From time to time, we may consider the {\em restriction} of an ultimate model to a set $R$ of predicates---the subset of the ultimate model consisting of all facts with predicates in $R$---denoted: $\mathcal{U}(P, E, ND)|_{R}$.
...
Note that ultimate models are always of finite cardinality because the only constant symbols that may appear in the ultimate model are those from the EDB, which is finite, and finitely many numbers generated by \dedalus{count}.  By the same reasoning, a \lang program may only have a finite number of different ultimate models.  As long as two stable models have the same nontrivial periodicities, they result in the same ultimate model, regardless of any other differences.
