...
For many distributed computations, achieving a determistic result for every input is desirable.  Recall that non-determinism only arises due to \dedalus{choice}, which only occurs in asynchronous rules to model network non-determinism.  Model-theoretically, a non-deterministic result is manifest in multiple ultimate models.

\begin{definition}
A \lang program is {\em confluent} (has a deterministic output) if, for every EDB, it has a unique ultimate model.  A non-confluent program is referred to as {\em diffluence}.
\end{definition}
...
not in another model, and $p$ must be persistent \wrm{eventually always true}, or it would not be
...
\begin{definition}
A fact in a persistent relation is a monotonic property. \wrm{what's a persistent relation?} \wrm{also, we should start from EDB, and say that's a persistent relation}
\end{definition}
...
\begin{definition}
A fact defined with even depth of negation over a monotonic property is also a monotonic property. \wrm{think this is better said as ``all paths through the PDG contain an even depth to the monotonic property''}
\end{definition}
...
\wrm{we'll be waiting for ever for counts of zero...}
...
\begin{theorem}
The coordinated program is confluent, and the ultimate model is the same as one in the original program(except the \dedalus{p\_send} stuff.
\end{theorem}
...
\wrm{notion of consistently sealed ultimate model; explain why preferable; point out PUM is a CSUM}
