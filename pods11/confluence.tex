...
Hence the lemma holds only for a conservative definition of monotonic properties.  
...
\begin{definition}
The {\em perfect ultimate model} of a \lang program with no negative cycles in its PDG, denoted $\mathcal{P}(P, E)$, is the ultimate model induced by ensuring a rule containing a \dedalus{!} or a \dedalus{count} is not satisfiable before the timestamp when the complete set of facts in the negated or aggregated predicates is sealed: a predicate \dedalus{p} is {\em sealed} at time \dedalus{t} if any fact in \dedalus{p} at time $\dedalus{s} > \dedalus{t}$ is also in \dedalus{p} at time \dedalus{t}.  
...
\begin{definition}
A {\em consistently sealed ultimate model} is an ultimate model generated by non-deterministically sealing the contents of a predicate.
\end{definition}
