...
contain syntactic negation \jmh{should we be mentioning aggregation?} and aggregation with
...
derivation depth) ?}  \jmh{No, sorry, that was just me trying unsuccessfully to make sense of your proof. Please follow through on this if it works, just telegraph your plans more clearly.}
...
\jmh{Is the idea that \dedalus{\#s} is a constant?  Or that it's \dedalus{\#S} and ranges over the domain of \dedalus{node}?  I don't like hash-marks in front of constants much, but maybe it's unavoidable?  Meanwhile, why do the bodies of the server rules not constrain the first attribute? Looks as if the server is peeking at data that's partitioning across many nodes.  I think this program needs debugging.}
...
For various uses of negation, we define a {\em perfect ultimate model}, and present a rewrite technique to convert a \lang program into a confluent program that computes this model.
...
\begin{definition}
A {\em consistently sealed ultimate model} is an ultimate model generated by non-deterministically committing to the current contents of a predicate.  \jmh{what do you mean ``committing to the current contents''?  say this in dedalus terminology?}
\end{definition}
