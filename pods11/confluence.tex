...
\end{lemma}
\begin{proof}
By reduction to the undecidable problem of determining whether a two-counter machine accepts any input.  See appendix.
\end{proof}
...
\begin{lemma}
\label{lem:monotonic-guarded-async-confluent}
\end{lemma}
\begin{proof}
\end{proof}
...
A relation is {\em tainted with asynchrony} if it transitively dependent on an \dedalus{async} predicate.

\begin{lemma}
A \lang program where every rule body has at most one relation tainted with asynchrony is confluent.
\end{lemma}
\begin{proof}
\end{proof}
...
Broadly, a \lang program is diffluent for 
...
\subsection{A simple algebra}

% \paa{add some nice text here transitioning the discussion into distributed systems etc}
% 
In practice, many natural distributed programs are not negation-free~\cite{cidr11}, and the conditions of Lemma~\ref{lem:guarding} do not hold.  Many interesting programs with negation are monotonic, and thus confluent.  In this section we broaden our static tests for monotonicity, enabling us to verify the confluence of a larger class of programs.

We begin by introducing a simple type for every predicate.  A predicate may have one of the following types: \dedalus{oneshot}, \dedalus{atomic}, \dedalus{ground}, \dedalus{monotonic}, \dedalus{antimonotonic}, or \dedalus{nonmonotonic}.  We will briefly explain each type.  A predicate is \dedalus{oneshot} if for every fact, if the fact exists at time $t$, then all other facts in the predicate must exist at time $t$.  A predicate is \dedalus{atomic} if for every fact, if the fact exists at time $t$, then all other facts in the predicate must exist at time $t$ and time $t+1$.  A predicate is \dedalus{ground} if every fact in the predicate exists at all times.  A predicate is \dedalus{monotonic} if for every fact, if the fact exists at time $t$, it exists at time $t+1$.  A predicate is \dedalus{antimonotonic} if for every fact, if the fact exists at time $t$, it exists at time $t-1$.  A predicate is \dedalus{nonmonotonic} if none of the previous definitions apply.

There are three operations in our algebra: join, negation, and async.  Note that the negation of an \dedalus{atomic} predicate is \dedalus{atomic}, the negation of a \dedalus{ground} predicate is \dedalus{ground}, and \dedalus{monotonic} and \dedalus{antimonotonic} are negatives of each other.

We will show that if all output predciates are \dedalus{monotonic}, then the program is guaranteed to be confluent.
...
all_responded() <- !not_responded();
not_responded() <- node(X), !response(X);
all_responded()@next <- all_responded();
\end{Dedalus}
\end{example}

Note that \dedalus{response_edb} is ground, \dedalus{response} is monotonic, \dedalus{all_responded} is atomic, because \dedalus{!not_responded} is oneshot, since \dedalus{!response} is antimonotonic and \dedalus{node} is static, and the body universally quantifies over the atomic set (generalization of ``it only has one fact, and it's persisted'').
...
is monotonic because it is {\em positive} (i.e. the negation depth is even to all non-static relations).  Note that the negation of a static relation is static, and thus it is monotonic ``by default''.
Note that the program is also confluent, because for any EDB, there is a unique ultimate model: either the EDB contains a response associated with every node, leading to a model with \dedalus{all\_responded()}, or there exists some node without an associated response in the EDB, leading to an empty ultimate model.

Note however that if the set \texttt{node} is not a static set, the program is not monotonic, because 
...
\jmh{
\end{comment}
The proof follows directly.  By the argument above, a monotonic Dedalus program has an ultimate model consisting of only monotonic properties, and by Lemma~\ref{lem:property} all such properties are true in all ultimate models.  Hence all ultimate models of a monotonic Dedalus program are the same.
...
The first rule---a local asynchronous rule\footnote{Note that this is actually a rule.  Its unsugared form is: \linebreak \dedalus{boomerang\_p(\#N) <- choice((N),(S)),time(S),node(N);}. This rule will only derive one fact per node, because each such derivation commits to a stable model in which the chosen time $S$ is functionally determined by the node address $N$.}---derives \dedalus{boomerang\_p()} at some unspecified time.  When \dedalus{boomerang\_p()} becomes true, we forever seal the \dedalus{p} set.  \dedalus{p\_send} facts accumulate until \dedalus{boomerang\_p()} is true, at which point \dedalus{p\_send} is copied to \dedalus{p}, where it is forever persisted, and deleted from \dedalus{p\_send}, atomically.  Since \dedalus{p} has no \dedalus{p\_send} facts until \dedalus{boomerang\_p()} is true, and no \dedalus{p\_send} facts are copied into \dedalus{p} after at any time after \dedalus{boomerang\_p()} first becomes true, predicate \dedalus{p} is sealed.  Thus we have:
