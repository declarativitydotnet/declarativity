...
Given a \lang program $P$ with no negative cycles in its PDG, we can generate a confluent program $P' = \textsc{Coord}(P)$ where $\mathcal{U}(P', E) = \mathcal{P}(P, E)$.
...
\begin{algorithmic}[0]
  ...
  \EndFor
  \State{add rules in Figure 1} %XXX
  \ForAll{rules with \dedalus{!p} in the body}
...
p_count_recv(Src,count<*>) <- p_send(Src,\dbar{X});
p(Y,\dbar{X}) <- p_send(Y,\dbar{X});
p(Y,\dbar{X})@next <- p(Y,\dbar{X});
p_incomplete() <- node(Src), !p_count_recv(Src, _);
p_incomplete() <- node(Src), !p_count_send(Src,_);
p_incomplete() <- p_count_recv(Src, C1),
                  p_count_send(Src, C2), C1 < C2;
\end{Dedalus}
...
\begin{theorem}
For any program $P$, $\textsc{Coord}(P)$ is confluent.
\end{theorem}
