...
For many distributed computations, achieving a determistic result for every input is desirable.  Recall that non-determinism in \lang only arises due to \dedalus{choice} in asynchronous rules, 
%which only occurs in asynchronous rules to model network non-determinism.  
...
\begin{definition}
A predicate is  {\em persistent} if any fact in the predicate is eventually henceforth true.
\end{definition}

Facts in persistent predicates are the only facts that appear in a program's ultimate models.
Persistence is a semantic notion, but in certain cases it is detectable from the program's 
syntax alone.  A rule of the form \dedalus{p(X)@next <- p(X);} clearly establishes \dedalus{p} as a persistent predicate for any EDB and execution in which \dedalus{p} is ever true. 
...
Note that facts in simply persistent relations cannot be retracted once they are asserted.  They begin false
...
\paa{hm, but a PDG is syntactic and we are defining these things in terms of derivations and facts}
\end{definition}
