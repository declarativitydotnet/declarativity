...
\begin{definition}
A \lang program has {\em guarded asynchrony} if all \dedalus{async} predicates \jmh{we have async rules, not async preds} are simply persisted.
\end{definition}
...
eventually succeed, \jmh{don't you need to define ``join success'' and prove that guarded asynchrony does what you say?} what it and by assumption, every {\em async}-derived predicate is guarded.
...
\begin{definition}
	\jmh{A monotonic property is a formula that...}
