...
An increasing amount of interest surrounds the use of logic langauges such as Datalog to ease designing and verifying the correctnes of large-scale asynchronous distributed systems.  While previous work demonstrated compactness of specification and efficiency of execution for practical systems, and resolved semantic difficulties, there has been limited prior work in applying the oft-praised analyzability of logic languages to the distributed systems domain, even though such applications have been conjectured.

We focus on, and define model-theoretic notions for {\em determinism}, {\em consistency}, and {\em eventual consistency}, three correctness criteria that are popular in the distributed systems domain.  Unfortunately, we prove that these are undecidable properties in the general case.  However, we prove the conjecture that {\em monotonicity} of logic can be a powerful conservative test for these criteria, and leverage existing {\em stratification} analyses from logic programming to enforce monotonicity through {\em coordination protocols} in a large class of non-monotonic programs.
