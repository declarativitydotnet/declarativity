...
We formalize the notion of replica consistency using Dedalus {\em constraints}.  In the remainder of this section, when we use the term consistency we refer to replica consistency. \wrm{someone please check this is true}

\begin{definition}
A Dedalus {\em constraint} is a rule whose head consists of a special predicate called \dedalus{fail()}.  We say a constraint is {\em violated} if it is satisfiable in any ultimate model.  We do not require constraints to unify on location attributes, as they are not ``executed'' by our operational formalism in Section~\ref{sec:operational}.  Instead, by ensuring the absence of \dedalus{fail()} from the ultimate model, we ensure that the operational formalism can never compute an ultimate model that violates the constraint.
\end{definition}

In order to talk about consistency, we need to differentiate between replicated
...
If a \lang program $P$ is eventually consistent with respect to a set of replicated predicates $E$, and replicas $R$, then we write that $(P, E, R)$ is eventually consistent.  We assume a unary predicate \dedalus{replica}, whose contents is exactly $R$.
...
If $P'$ is confluent, we say that $P$ is {\em downstream confluent} with respect to \dedalus{p}.  If $\mathcal{U}(P', E) = \mathcal{U}(P', E')$, for all $E'$ where the symmetric difference $E' \Delta E$ contains only non-replicated predicates, we say $P$ is {\em symmetric} w.r.t. unreplicated data.
\end{definition}
...
\begin{theorem}
If a program $P$, which has replicated predicates $E$ is downstream confluent with respect to all $\dedalus{p} \in E$, and furthermore $P$ is symmetric w.r.t. unreplicated data, then $P$ is eventually consistent.
\end{theorem}
\begin{proof}
\wrm{Easy. To fill in later.}
\end{proof}

Note that the converse does not hold.

An example of a replication protocol is shown:
...
We can instrument an arbitrary \lang program to enable eventual consistency by inserting a replication rule for all async predicates \dedalus{p}, provided that the program is downstream confluent for all such \dedalus{p}.  \wrm{Inserting this replication rule ensures symmery.}

Procedure $\textsc{EC}(P)$ makes an arbitrary \lang program eventually consistent by doing the following for each async predicate \dedalus{p}:

1. Inserting a replication rule.
2. Replace any references to \dedalus{p} in rule subogals to \dedalus{rep\_p}.

\wrm{Argue this makes it EC}
