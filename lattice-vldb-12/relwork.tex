...
schemes allow users to specify that certain operations can be commuted
with other operations; this enlarges the space of allowed transaction schedules,
...
designs~\cite{Shapiro2011a}. CRDTs and \lang lattices often employ similar
design patterns. As noted in Section~\ref{sec:intro}, the CRDT approach
considers the correctness of replicated data types in isolation. Hence, even if
a CRDT is correct, application-level state may remain inconsistent.
...
admits only the class of so-called ``stratified programs''~\cite{Apt1988}. Many
