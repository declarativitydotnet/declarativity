...
\nrc{TODO}
In a statement where the rhs and lhs are both lattice elements, the
interpretation of the statement is that the rhs element should be ``merged
into'' the lhs element using the least upper bound operator ($\lor$) provided by
the lattice.
...
A method's signature indicates its monotonicity properties. More precisely,
\lang distinguishes between methods that are monotone and a subset of those
methods that are \emph{morphisms}. The properties a morphism must satisfy are
given formally in Section~\ref{sec:lattice-defn}, but the intuition is that
...
Every lattice defines a non-monotonic method named \texttt{reveal} which returns
a representation of the lattice element as a plain Ruby value. For example, the
\texttt{reveal} method on a \texttt{lset} lattice returns a representation of
the set as a Ruby array. This is non-monotonic because \nrc{TODO}
