...
same.\footnote{Bloom facts can be deleted, but this is an explicit non-monotonic
  ...
  of a new fact~\cite{dedalus}.} Consider a simple \lang rule that derives tuples
with a lattice element as an attribute value:
\begin{verbatim}
    t1 <= t2 {|t| [t.x, lat_foo]}
\end{verbatim}
where \texttt{t1} and \texttt{t2} are Bloom relations and \texttt{lat\_foo}
identifies a lattice, and suppose that the first column of \texttt{t1} is the
...
One way to avoid this problem would be to require that lattice elements embedded
into Bloom collections be computed exactly before the rule that performs the
...
also seems unsatisfying to require stratification of a monotonic program.

Instead, \lang allows lattice embeddings to produce multiple tuples and then
merges those tuples together during the course of the fixpoint computation.
