...
has a partial order $<_S$, the least upper bound is defined for all elements
$x,y \in S$. The distinguished element $\bot$ is the smallest element in $S$;
this implies that $x \lor \bot = x$ for all $x \in S$. For brevity, we use the
term ``lattice'' to mean ``bounded join semilattice'' in the rest of this paper.
...
$g: S \to T$ such that $\forall a,b \in S: a \leq_S b \Rightarrow g(a) \leq_T
...
programming, a lattice is a class that obeys a certain interface and a lattice
element is an instance of that class.

In \lang programs, lattices are used in a similar fashion to collections. For
example, line~\ref{line:quorum-lset-decl} of Figure~\ref{fig:lattice-quorum}
declares that the \texttt{votes} identifier will be associated with elements of
