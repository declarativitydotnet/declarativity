...
Section~\ref{sec:lattice-defn}). The input to a monotone lattice method is the
...
computed exactly before allowing the non-monotonic operator to be invoked. This
...
Every lattice defines a non-monotonic \texttt{reveal} method which returns a
...
\texttt{gt(i)} $\to$ \texttt{lbool}\newline
\texttt{gt\_eq(i)} $\to$ \texttt{lbool}\newline
\texttt{$\mathtt{+}$(lmax)} $\to$ \texttt{lmax}\newline
\texttt{$\mathtt{-}{}$(lmax)} $\to$ \texttt{lmax} & \\
\hline
\texttt{lmin} & Min over an ordered domain & $\infty$ & $min(a, b)$ &
\texttt{lt(i)} $\to$ \texttt{lbool}\newline
\texttt{lt\_eq(i)} $\to$ \texttt{lbool}\newline
\texttt{$\mathtt{+}$(lmin)} $\to$ \texttt{lmin}\newline
\texttt{$\mathtt{-}{}$(lmin)} $\to$ \texttt{lmin} & \\
\hline
\texttt{lset} & Set of values & $\emptyset$ & $a \cup b$ &
...
\hline
\texttt{lbag} & Multiset of values && $a \cup b$ &
...
satisfied, remain satisfied. An \texttt{lbool} instance is returned by several
of the morphisms provided by the builtin lattices. For example, the \texttt{gt}
morphism on the \texttt{lmax} lattice returns an \texttt{lbool}
value. \texttt{gt} takes a constant integer argument $k$; once an \texttt{lmax}
value exceeds $k$, it will remain $>k$ as the set grows. The \texttt{when\_true}
morphism takes a Ruby block; when the lattice element has the value
\texttt{true}, the block is evaluated. For example, see
line~\ref{line:quorum-when-true} in Figure~\ref{fig:lattice-quorum}. The
\texttt{when\_true} operator is similar to an ``if'' statement---observe that
supporting an ``if-else'' statement would require non-monotonicity.
...
use an \texttt{lmap} to associate each session ID with an \texttt{lset}
containing the webpages visited by that session. The \texttt{lmap} merge
function takes the union of the key sets of the two input maps. If a key occurs
in both maps, the two corresponding values are merged using the appropriate
...
\texttt{lset} lattice, $\{1,2\}$ and $\{2,3\}$. For \texttt{size} to be a
morphism, this equality would need to hold: $size(\{1,2\} \lor_{\mathtt{lset}}
\{2,3\}) = size(\{1,2\}) \lor_{\mathtt{lmax}} size(\{2,3\})$.
