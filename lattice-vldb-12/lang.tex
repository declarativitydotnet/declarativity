...
(e.g., the causal delivery protocol described in Section~\ref{sec:causal}). The
behavior of most of the lattice methods should be unsurprising, so we do not
exhaustively describe every method in this section; rather, we highlight a few
lattices that deserve further comment.
...
\texttt{lmax} lattice returns an element of the \texttt{lbool} lattice because
once the maximum of a set is $>k$, it will remain $>k$ as the set grows. The
\texttt{when\_true} morphism takes a Ruby block; when the lattice element has
the value \texttt{true}, the block is evaluated. For example, see
line~\ref{line:quorum-when-true} in Figure~\ref{fig:lattice-quorum}.

Note that \emph{size} is a monotone function provided by several lattices but it
is not a morphism. This is because \texttt{size} cannot be distributed over the
merge functions of those lattices. For example, consider two elements of the
