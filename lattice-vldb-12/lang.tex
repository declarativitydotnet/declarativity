...
\subsection{User-defined lattice types}
\begin{figure}[t]
\begin{scriptsize}
\begin{lstlisting}
...
While the builtin lattice types provided by \lang will suffice for many
applications, developers can also create custom lattice definitions to capture
domain-specific behavior. To define a new lattice, a developer creates a Ruby
class that meets a certain API contract. The Ruby class that defines the
\texttt{lset} lattice can be seen in Figure~\ref{fig:lattice-lset}.
...
\item \texttt{merge(e)}: given a lattice element $e$, this method returns the
  ...
  algebraic properties summarized in Section~\ref{sec:lattice-defn}---in
...
any number of monotone functions, morphisms, and non-monotonic methods.

Because Ruby is dynamically typed, there will typically be constraints on the
legal inputs to these functions that cannot be enforced until runtime. For
example, initializing a \texttt{lbool} lattice with a non-boolean value is not
...
programming
languages~\cite{Okasaki1999}. % XXX: maybe not the right place for this
