...
different values for the second attribute, which would violate \texttt{t1}'s
...
deterministic and is the least upper bound of all the merged values.

Nevertheless, we felt this behavior should be manifest in the program syntax,
because tuples in Datalog relations are traditionally immutable. Hence, to
enable this ``lattice merge'' behavior for a column in a collection, the
collection's name should be preceded with ``\texttt{!}'' when the collection is
declared (e.g.,line~\ref{line:ddl-lattice-merge} in
Figure~\ref{fig:vector-clock-src}). If the annotation is not specifier, lattice
