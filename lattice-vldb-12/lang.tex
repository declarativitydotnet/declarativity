...
module interfaces) as collections with special semantics, and it would be
...
value from the lattice element (via the \texttt{reveal} method).  However, that
would introduce needless non-monotonicity into the program.
...
\begin{verbatim}
    t1 <= t2 {|t| [t.x, lat_foo]}
\end{verbatim}
where \texttt{t1} and \texttt{t2} are Bloom collections, \texttt{lat\_foo}
identifies a lattice, and the first column of \texttt{t1} is the collection's
key. The value associated with \texttt{lat\_foo} can change over the course of a
single timestep (specifically, \texttt{lat\_foo} can grow ``upward'' according
to the lattice's partial order). Implemented naively, this might result in
multiple \texttt{t1} tuples with different values for the second attribute,
which would violate \texttt{t1}'s key (the first column of \texttt{t1} would not
functionally determine a single value for the second column).
