...
other nodes in the system. Vector clocks can be used to assign a partial order
over the events in the system that agrees with the causal relationship between
events.
...
We would not expect an implementation of vector clocks to be confluent;
accordingly, Figure~\ref{fig:vector-clock-src} is not a monotonic \lang program
because of the use of \texttt{scratch} collections
...
timestamp. Although the vector clock program is not confluent, all the
statements in the program apply monotone lattice methods. Hence, \texttt{my\_vc}
increases monotonically and \texttt{next\_vc} is at least as large as
...
Note that because none of the statements in Figure~\ref{fig:vector-clock-src}
invokes a non-monotonic operator, the statements can be executed in any
order. The ``lattice embedding'' feature described in
Section~\ref{sec:lattice-embedding} ensures that the \texttt{out\_msg\_vc} tuple
contains a single vector clock that reflects all the messages received in the
current timestep.
