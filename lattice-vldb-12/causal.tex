...
embedded as a column of a collection that does not allow ``lattice merges''. \jmh{Justify the preceding sentence via the program syntax!} So
...
that a node's \texttt{ord\_buf} strictly increases over time.  \jmh{Can't you say that the ord\_buf in in\_msg grows monotonically, which implies that delivery does indeed respect the causal constraint?)}
