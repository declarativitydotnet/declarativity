...
\emph{intended} to be confluent. Rather than insisting on a single outcome
despite message delays, these protocols allow timing conditions to affect the
choice among several acceptable outcomes. For example, in a vector clock system,
the particular clock values assigned to messages differ depending on network
non-determinism. Nevertheless, the system does not exhibit arbitrary
...
which maps to our intuitive notion these protocols make ``progress'' over
time. These are similar to the interface guarantees provided by CRDTs, with the
additional benefit that monotone functions allow safe mappings between lattice
values.

As a secondary benefit, we observe that our programs are readable and concise
(e.g., 20x shorter than a comparable Java program). This suggests that \lang is
suitable for common distributed programming tasks.
