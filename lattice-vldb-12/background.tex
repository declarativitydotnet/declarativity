...
normal logical deduction: that is, the lhs and rhs are true at the ``same''
...
next timestep. The \verb+<~+ operator specifies that the rhs will be merged into
the lhs collection at some non-deterministic future time. The lhs of a \verb+<~+
rule must be a channel; the \verb+<~+ operator models asynchronous messaging.
