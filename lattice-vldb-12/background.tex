...
fashion: the set of received votes grows and the count of the \texttt{votes}
...
line~\ref{line:bloom-nm-quorum}). To solve this problem, we need to introduce a
notion of program outputs that ``grow over time'' according to a partial order
that is not set containment. We do this by extending Bloom to allow
\emph{lattices}, as we describe in the next section.
