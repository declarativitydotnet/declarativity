...
a ``side effect,'' such as modifying local state or sending a network message to
another Bloom instance.  These effects are deferred during the second phase of
the timestep; the third phase is devoted to carrying them out.
...
\emph{confluent}: given a fixed input, all program executions result in the same
...
\emph{points of order}: program locations at which the output of an
asynchronously derived value is consumed by a non-monotonic operator.  This is
because asynchronous messaging results in non-deterministic arrival order, and
...
their inputs.  To achieve consistency, the programmer either needs to rewrite
