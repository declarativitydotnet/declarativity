...
\begin{itemize}
  ...
  \item \textbf{Bloom programming is real nice now}:  The idea of using logic was to give us ``disorderly'' distributed programming a la MapReduce and SQL.  The convergent object approach went after the same goal from a more imperative perspective---``safe'' objects.  Both have their merits.  \lang's roots in logic still encourage set-wise, disorderly thinking, and the core construct of joining streams of events/messages with collections of stored facts is a nice way to capture asynchronous programming.  But stuff like counters are so very natural...
\end{itemize}
