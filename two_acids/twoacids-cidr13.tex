...
\title{{\ttlit Aqua Regia}: A Solution of Two ACIDs for Bespoke Systems\titlenote{\emph{Aqua regia}
...
\begin{abstract}
  ...
  notion of \emph{Non-Monotone Serializability} that a compiler can synthesize in
  an application-aware fashion.  We then show how traditional ACID 
  ideas almost inescapably inform some of the most important semantic/performance design 
  tradeoffs of practical modern systems in the ACID 2.0 context.
  Finally, given this discussion of software sythesis, we look toward a future of custom-tailored
 \emph{Bespoke} systems, in which application-aware language designs and compiler analysis enable the 
  synthesis of custom low-level code.
\end{abstract}
...
We begin by applying ACID 2.0 ideas to traditional ACID.  Specifically, we use the CALM theorem to generalize the traditional notion of conflicting actions.  We then propose compiler analysis techniques that can automatically identify high-level semantic concurrency opportunities, allowing wait-free behaviors that would be forbidden by traditional Read/Write transaction protocols.
...
From the other direction, CALM also generalizes the notion of ``non-conflicting'' actions beyond Reads: any monotonic logic can be reordered, so transaction scheduling need only consider semantically non-monotonic actions as the basis of conflicts.
...
Once its inputs are sealed, we can evaluate a non-monotonic operation and proceed with monotonic reasoning regarding its outcome.
