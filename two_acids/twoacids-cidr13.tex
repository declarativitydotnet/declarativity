...
NOT(revoked). The CALM Theorem shows that this non-monotonicity might produce
different outcomes depending on the order in which operations are scheduled;
hence, a concurrency control scheme is needed to constrain the allowed operation
orders to ensure that only serializable schedules are produced.

To make this discussion more concrete, we first show how traditional read/write
schedules can be encoded in a stateful logic such as Dedalus~\cite{dedalus}. We
...
this as \emph{Non-Monotone Serializability}.

Consider the classic case of credit/debit ledgers. For the sake of exposition,
we assume a single account and two transactions, \texttt{T1} and \texttt{T2}.
For now we assume only credit operations and consider debits shortly. Using
traditional Read and Write operations, we can model this system using Dedalus as
follows:
\begin{quote}
\emph{// Balance persists at next timestep unless revoked}\\
bal(X)@next :- bal(X), $\neg$revoke\_bal(X).\\
\\
t1\_read(X)@T1\_R\_TIME :- bal(X)@T1\_R\_TIME.\\
t1\_write(X+1)@T1\_W\_TIME :- t1\_read(X)@T1\_R\_TIME.\\
\\
t2\_read(X)@T2\_R\_TIME :- bal(X)@T2\_R\_TIME.\\
t2\_write(X+1)@T2\_W\_TIME :- t2\_read(X)@T2\_R\_TIME.\\
\\
\emph{// Apply writes to database}\\
bal(X)@next :- t1\_write(X).\\
revoke\_bal(X) :- t1\_write(\_), bal(X).\\
bal(X)@next :- t2\_write(X).\\
revoke\_bal(X) :- t2\_write(\_), bal(X).\\
\end{quote}
Note that \texttt{p(X)@t} asserts that a fact \texttt{p(X)} is true at time
\texttt{t}; \texttt{@next} denotes the timestamp that immediately follows the
timestamp of the body predicates. The special timestamps \texttt{T1\_R\_TIME}
and related values denote the time at which the read and write operations are
occur; these are chosen by the scheduler, and hence are non-deterministic. The
conflicts in this schedule are manifest from the program syntax: negation
($\neg$ in line X) is non-monotonic. Hence, concurrency control is required to
select a legal assignment of read and write timestamps.

We can avoid the conflicts in this schedule by making updates monotonic. In a
credit-only system, the balance is a monotonically increasing number; hence, we
can model the account balance as a join semilattice with a least upper bound
(``$\sqcup$'') that takes the largest balance proposed so
far~\cite{Conway2012}. This \emph{replaces} the previous non-monotonic
persistence rule: rather than revoking and asserting new account balances,
instead the balance will grow as more credit operations are received.
\begin{quote}
\emph{// Balance is least upper bound of proposed credits}
bal(X)@n :- $\sqcup_{i=0}^n$ proposed\_bal(\_, X)@i.

proposed\_bal(``T1'', X + 1)@next :- bal(X)@T1\_W\_TIME.
proposed\_bal(``T2'', X + 1)@next :- bal(X)@T2\_W\_TIME.
\end{quote}
Because this program contains only monotonic constructs, the CALM Theorem
certifies that it is deterministic regardless of the timestamps that are
assigned to the write operations; thus, it can safely be executed without
concurrency control. Said differently, \emph{monotone logic is conflict-free}.

We now consider how to support debit operations that succeed only if the account
would remain liquid: ``conflicts'' can arise between the liquidity checks of
debits and credits. From the ACID 2.0 perspective, debits can introduce
conflicts because they are non-monotonic: monotone operations can only cause the
\texttt{bal} lattice to increase. Hence, we must establish ordering constraints
for each debit with respect to preceding credits.  However, the order among
credits remains irrelevant---the only order that matters relates to the
non-monotone operator ``debit.'' Importantly, the non-monotonicity of debits is
clear from the program syntax; hence, a compiler can observe the presence of a
potentially conflicting operation, and insert the necessary
coordination~\cite{Alvaro2011}.

Surprisingly, we can make debits monotonic by maintaining a separate ``debit
ledger''; both debit and credit ledgers would then grow monotonically. However,
checking the liquidity of an account is \emph{not} monotonic because subtracting
one monotonically increasing number from another does not, in general, yield a
monotonically increasing result. This design moves the coordination cost from
debit operations to liquidity checks, at the expense of allowing some debits to
be performed on illiquid accounts; nevertheless, similar designs are often
